...
Note that while using your configuration file, there are three things to pay attention to:
   1. In the `elements` section, there is no `mixInputs` key, and `outputs` key, but `RF_inputs` and `RF_outputs` correspondingly instead.
   2. There is a new key named `octaves`, where all the up-converter and down-converter parameters are configured.
...
1. `OctaveUnit` class:
   * Define the name, IP address, port and clock setting.
