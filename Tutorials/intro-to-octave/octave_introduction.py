"""
octave_introduction.py: shows the basic commands to control the octave's clock, synthesizers, up-converters, triggers, down-converters and calibration
"""
...
# You can connect clock out from rear panel to an oscilloscope to see the 1GHz signal
...
for i in range(len(elements)):
    qm.octave.set_lo_source(elements[i], OctaveLOSource.Internal)  # Use the internal synthetizer to generate the LO.
    qm.octave.set_lo_frequency(elements[i], LO)  # assign the LO inside the octave to element
    qm.octave.set_rf_output_gain(elements[i], 0)  # can set the gain from -10dB to 20dB
    qm.octave.set_rf_output_mode(elements[i], RFOutputMode.on)  # set the behaviour of the RF switch to be 'on'.
