...
if node.parameters.simulate:
    ...
    ds = fetch_results_as_xarray(job.result_handles, qubits, {"time": time_axis, "qubit_init_state": range(2), "shot_index": range(node.parameters.num_averages)})
    # Convert raw ADC traces into volts
    ds = ds.assign({key: -ds[key] / 2**12 for key in ("adcI", "adcQ")})
