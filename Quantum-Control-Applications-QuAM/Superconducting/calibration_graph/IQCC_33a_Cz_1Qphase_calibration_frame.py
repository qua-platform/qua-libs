...
node = QualibrationNode(name="IQCC_33a_Cz_1Qphase_calibration_frame", parameters=Parameters())
...
sweep_axes = {"qubit": xr.DataArray([qp.name for qp in qubit_pairs]), "frame": xr.DataArray(frames)}
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_(*from_array(frame, frames)):
                for qubit, state_q, state_st in [
                    (qp.qubit_control, state_control[i], state_st_control[i]),
                    (qp.qubit_target, state_target[i], state_st_target[i]),
                ]:
                    # reset
...
node.results = {
    "ds": ds,
    "phases_target": phases_target,
    "phases_control": phases_control,
    "figure_phase": node.results["figure_phase"],
