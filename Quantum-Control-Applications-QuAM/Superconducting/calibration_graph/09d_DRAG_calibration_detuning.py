...
class Parameters(NodeParameters):
    ...
    frequency_detuning_span_in_mhz: float = 10
    frequency_detuning_step_in_mhz: float = 0.5
    min_wait_time_in_ns: int = 16
    max_wait_time_in_ns: int = 3000
    num_time_points: int = 400
...
node = QualibrationNode(name="09d_DRAG_calibration_detuning", parameters=Parameters())
node_id = get_node_id()
...
span = node.parameters.frequency_detuning_span_in_mhz * u.MHz
step = node.parameters.frequency_detuning_step_in_mhz * u.MHz
...
with program() as ramsey:
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_each_(t, idle_times):
                ...
                with for_(*from_array(df, dfs)):
                    assign(phi, Cast.mul_fixed_by_int(1e-9, 4 * t * df))
    ...
    with stream_processing():
        ...
        for i in range(num_qubits):
            if node.parameters.use_state_discrimination:
                state_st[i].buffer(len(dfs)).buffer(len(idle_times)).average().save(f"state{i + 1}")
            else:
                I_st[i].buffer(len(dfs)).buffer(len(idle_times)).average().save(f"I{i + 1}")
                Q_st[i].buffer(len(dfs)).buffer(len(idle_times)).average().save(f"Q{i + 1}")
...
if not node.parameters.simulate:
    if node.parameters.load_data_id is None:
        ...
        ds = fetch_results_as_xarray(job.result_handles, qubits, {"detuning": dfs, "time": idle_times})
    ...
    for ax, qubit in grid_iter(grid):
        if node.parameters.use_state_discrimination:
            ds.sel(sign=1).loc[qubit].state.plot(
                ax=ax, x="detuning", c="C0", marker=".", ms=5.0, ls="", label="$\Delta$ = +"
            )
            ds.sel(sign=-1).loc[qubit].state.plot(
            ...
            (decay.loc[qubit]).plot(
                ax=ax, x="detuning", c="C0", marker=".", ms=5.0, ls="-"
            )
    ...
    for df in ds.detuning:
        (ds.loc[qubit].I * 1e3).sel(detuning=df).plot(
                x="time", c="C0", marker=".", ms=5.0, ls=""
        )
