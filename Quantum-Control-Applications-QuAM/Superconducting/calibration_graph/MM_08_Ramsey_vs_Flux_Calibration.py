...
# Convert frequency detuning (MHz) to detuning in Hz for virtual Z-rotations
...
with program() as ramsey:
    ...
    phi = declare(fixed)  # QUA variable for dephasing (virtual Z-rotation)
    flux = declare(fixed)  # QUA variable for flux dc level

    for i, qubit in enumerate(qubits):
        # Set qubit frequency via flux bias
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_(*from_array(flux, fluxes)):
                with for_(*from_array(t, idle_times)):
                    # Implement virtual Z-rotation: calculate phase shift
                    # 4*t because idle time is in clock cycles (4 ns) and convert to seconds
                    ...
                    # Reset qubit frame to avoid accumulated rotations
