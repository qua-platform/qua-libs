...
# Define sweep axes for the data fetcher
sweep_axes = {"qubits": xr.DataArray([q.name for q in qubits]), "N": xr.DataArray(np.arange(n_runs))}
...
with qm_session(qmm, config, timeout=node.parameters.timeout) as qm:
    ...
    for ds in data_fetcher:
        progress_counter(data_fetcher["n"], n_runs, start_time=data_fetcher.t_start)
...
for ax, qubit in grid_iter(grid):
    ...
    ax.plot(
        1e3
        * (ds.I_g.sel(qubit=qn) * np.cos(results[qn]["angle"]) - ds.Q_g.sel(qubit=qn) * np.sin(results[qn]["angle"])),
        1e3
        * (ds.I_g.sel(qubit=qn) * np.sin(results[qn]["angle"]) + ds.Q_g.sel(qubit=qn) * np.cos(results[qn]["angle"])),
        ".",
        alpha=0.2,
        label="Ground",
        markersize=1,
    ...
    ax.plot(
        1e3
        * (ds.I_e.sel(qubit=qn) * np.cos(results[qn]["angle"]) - ds.Q_e.sel(qubit=qn) * np.sin(results[qn]["angle"])),
        1e3
        * (ds.I_e.sel(qubit=qn) * np.sin(results[qn]["angle"]) + ds.Q_e.sel(qubit=qn) * np.cos(results[qn]["angle"])),
        ".",
        alpha=0.2,
        label="Excited",
        markersize=1,
    )
    ax.axvline(1e3 * results[qn]["rus_threshold"], color="k", linestyle="--", lw=0.5, label="RUS Threshold")
    ax.axvline(1e3 * results[qn]["threshold"], color="r", linestyle="--", lw=0.5, label="Threshold")
    ax.axis("equal")
    ax.set_xlabel("I [mV]")
    ax.set_ylabel("Q [mV]")
...
plt.tight_layout()
results["figure_IQ_blobs"] = grid.fig

# Plot fidelity matrix
grid = QubitGrid(ds, [q.grid_location for q in qubits])
for ax, qubit in grid_iter(grid):
    confusion = results[qubit["qubit"]]["confusion_matrix"]
    ax.imshow(confusion)
    ax.set_xticks([0, 1])
    ax.set_yticks([0, 1])
...
plt.tight_layout()
results["figure_fidelity"] = grid.fig

# %% {Update_state}
if node.parameters.load_data_id is None:
    with node.record_state_updates():
        for qubit in qubits:
            qubit.resonator.operations[operation_name].integration_weights_angle -= float(results[qubit.name]["angle"])
            # Convert the thresholds back in demod units
            qubit.resonator.operations[operation_name].threshold = (
                float(results[qubit.name]["threshold"]) * qubit.resonator.operations[operation_name].length / 2**12
            )
            qubit.resonator.operations[operation_name].rus_exit_threshold = (
                float(results[qubit.name]["rus_threshold"]) * qubit.resonator.operations[operation_name].length / 2**12
            )
            if operation_name == "readout":
                qubit.resonator.confusion_matrix = results[qubit.name]["confusion_matrix"].tolist()

# %% {Save_results}
node.results = {"ds": ds, "results": results, "initial_parameters": node.parameters.model_dump()}
