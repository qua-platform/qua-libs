...
from quam_libs.components.gates.two_qubit_gates import CZGate_SNZ_2elements, CZWithCompensationGate
from quam_libs.lib.pulses import FluxPulse, SNZPulse_main, SNZPulse_pixel
...
def generate_snz_config(config, rel_vals: list, spacing: int):
    for qp in qubit_pairs:
        ...
        pulse = SNZPulse_main(
            ...
            step_length=1,
            spacing=spacing,
            id=f"CZ_snz_main_{qp.qubit_target.name}",
        )
        ...
        pulse = SNZPulse_pixel(
            amplitude=amplitude,
            step_amplitude=amplitude,
            length=length + node.parameters.extra_flux_pulse_length,
            step_length=1,
            spacing=spacing,
            id=f"CZ_snz_pixel_{qp.qubit_target.name}",
        )
        pulse.parent = qp.qubit_control.z
        pulse.apply_to_config(config)
        plt.plot(pulse.waveform_function())
        config["elements"][qp.qubit_control.z2.name]["operations"][pulse.id] = pulse.pulse_name
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_(*from_array(frame, frames)):
                ...
                with for_(*from_array(amplitude, amplitudes)):
                    ...
                    with for_(*from_array(rel_val, rel_vals)):
                        ...
                        with for_(*from_array(control_initial, [0, 1])):
                            ...
                            qp.qubit_control.z2.wait(qp.qubit_target.xy.operations["x90"].length + 5)
                            ...
                            qp.qubit_control.z.play(
                                f"CZ_snz_main_{qp.qubit_target.name}",
                                amplitude_scale=amplitude,
                                validate=False,
                            )

                            qp.qubit_control.z2.play(
                                f"CZ_snz_pixel_{qp.qubit_target.name}",
                                amplitude_scale=amplitude*rel_val,
                                validate=False,
                            )
                            qp.align()
...
if not node.parameters.simulate:
    if node.parameters.load_data_id is None:
        with node.record_state_updates():
            for qp in qubit_pairs:
                qp.gates["Cz_SNZ_2elements"] = CZGate_SNZ_2elements(
                    flux_pulse_control=SNZPulse_main(
                        length=qp.gates["Cz_unipolar"].flux_pulse_control.length
                        ...
                        * min_coords[qp.name].rel_val.values[0].tolist(),
                        ...
                        id="CZ_snz_main_" + qp.qubit_target.name,
                    ),
                    flux_pulse_control_2=SNZPulse_pixel(
                        length=qp.gates["Cz_unipolar"].flux_pulse_control.length
                        + node.parameters.extra_flux_pulse_length,
                        amplitude=min_coords[qp.name].amp_full.values.tolist(),
                        step_amplitude=min_coords[qp.name].amp_full.values.tolist()
                        * min_coords[qp.name].rel_val.values[0].tolist(),
                        step_length=1,
                        spacing=node.parameters.spacing,
                        id="CZ_snz_pixel_" + qp.qubit_target.name,
                    )
                )
                qp.gates["Cz"] = f"#./Cz_SNZ_2elements"
