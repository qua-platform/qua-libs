...
class Parameters(NodeParameters):
    ...
    qubit_pairs: Optional[List[str]] = ["coupler_qA2_qA1"]
...
node = QualibrationNode(name="03a_Qubit_Spectroscopy_coupler", parameters=Parameters())
node_id = get_node_id()
...
with program() as qubit_spec:
    ...
    for i, qp in enumerate(qubit_pairs):
        ...
        machine.set_all_fluxes(flux_point=flux_point, target=qp.qubit_target)
        ...
        with for_(n, 0, n < n_avg, n + 1):
            ...
            with for_(*from_array(df, dfs)):
                ...
                qubit_control.xy.update_frequency(df + qubit_control.xy.intermediate_frequency)
                qubit_control.align()
                duration = operation_len * u.ns if operation_len is not None else (qubit_control.xy.operations[operation].length + qubit_control.z.settle_time) * u.ns
                # Bring the qubit to the desired point during the saturation pulse
                # qubit_control.z.play("const", amplitude_scale=arb_flux_bias_offset[qubit_control.name] / qubit_control.z.operations["const"].amplitude, duration=duration)
                # Play the saturation pulse
                qubit_control.xy.wait(qubit_control.z.settle_time * u.ns)
                ...
                qubit_target.resonator.measure("readout", qua_vars=(I[i], Q[i]))
                # Wait for the qubit to decay to the ground state
                qubit_target.resonator.wait(machine.depletion_time * u.ns)
                # save data
                save(I[i], I_st[i])
                save(Q[i], Q_st[i])
...
if not node.parameters.simulate:
        ...
        ds = ds.assign_coords(
            {
                "freq_full": (
                    ["qubit", "freq"],
                    np.array([dfs + qp.qubit_control.xy.RF_frequency for qp in qubit_pairs]),
                )
            }
        )
    ...
    abs_freqs = dict(
        [
            (
                qp.name,
                ds.freq_full.sel(freq = result.position.sel(qubit=qp.name).values).sel(qubit=qp.name).values,
            )
            for qp in qubit_pairs if not np.isnan(result.sel(qubit=qp.name).position.values)
        ]
    )
