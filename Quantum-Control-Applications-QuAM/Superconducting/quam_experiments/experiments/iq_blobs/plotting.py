...
def plot_iq_blobs(ds: xr.Dataset, qubits: List[AnyTransmon], fits: xr.Dataset):
    ...
    grid.fig.legend(handles, labels, loc="lower center", ncol=4)
...
def plot_individual_iq_blobs(ax: Axes, ds: xr.Dataset, qubit: dict[str, str], fit: xr.Dataset = None):
    ...
    ax.plot(1e3 * fit.Ie_rot, 1e3 * fit.Qe_rot, ".", alpha=0.2, label="Excited", markersize=1)
    ax.axvline(1e3 * fit.rus_threshold, color="k", linestyle="--", lw=0.5, label="RUS Threshold")
