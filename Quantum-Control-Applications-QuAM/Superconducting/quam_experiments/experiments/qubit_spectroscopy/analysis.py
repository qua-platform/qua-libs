...
def fit_raw_data(
    ...
    shifts = np.abs((ds_fit.IQ_abs - ds_fit.IQ_abs.mean(dim="detuning"))).idxmax(
        dim="detuning"
    )
    ...
    ds_fit = ds_fit.assign(
        {
            "I_rot": ds_fit.I * np.cos(ds_fit.iw_angle)
            + ds_fit.Q * np.sin(ds_fit.iw_angle)
        }
    )
...
def _extract_relevant_fit_parameters(fit: xr.Dataset, node: QualibrationNode):
    ...
    x180_length = np.array(
        [q.xy.operations["x180"].length * 1e-9 for q in node.namespace["qubits"]]
    )
    used_amp = np.array(
        [
            q.xy.operations["saturation"].amplitude
            * node.parameters.operation_amplitude_prefactor
            for q in node.namespace["qubits"]
        ]
    )
    ...
    fit = fit.assign(
        {
            "saturation_amplitude": factor_cw
            * used_amp
            / node.parameters.operation_amplitude_prefactor
        }
    )
    ...
    saturation_amp_success = (
        np.abs(fit.saturation_amplitude) < limits[0].max_wf_amplitude
    )
