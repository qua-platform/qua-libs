...
class SimulatableNodeParameters(RunnableParameters):
    ...
    simulation_duration_ns: int = Field(
        default=2500, description="Duration over which the simulation will collect samples"
    )
    use_waveform_report: bool = Field(
        default=False, description="Whether to use the interactive waveform report in simulation"
    )
