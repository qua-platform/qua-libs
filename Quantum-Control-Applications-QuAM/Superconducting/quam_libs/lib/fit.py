...
def fit_oscillation_decay_exp(da, dim):
    ...
    return fit_res.assign_coords(fit_vals=('fit_vals', ['a', 'f', 'phi', 'offset', 'decay',
                                                        ...
                                                        'offset_a', 'offset_f', 'offset_phi', 'offset_offset',
                                                        'offset_decay',
                                                        'decay_a', 'decay_f', 'decay_phi', 'decay_offset',
                                                        'decay_decay']))
    ...
    def apply_fit(x, y, a, f, phi, offset):
        try:
            ...
            fit = model.fit(y, t=x, a=Parameter("a", value=a, min=0), f=Parameter(
                "f", value=f, min=np.abs(0.5 * f), max=np.abs(f * 3 + 1e-3)), phi=Parameter('phi', value=phi),
                            offset=offset)
            if fit.rsquared < 0.9:
                fit = model.fit(y, t=x, a=Parameter("a", value=a, min=0), f=Parameter(
                    "f", value=1.0 / (np.max(x) - np.min(x)), min=0, max=np.abs(f * 3 + 1e-3)),
                                phi=Parameter('phi', value=phi), offset=offset)
...
def peaks_dips(da, dim, prominence_factor=5, number=1, remove_baseline=True) -> xr.Dataset:
    ...
    base_line = xr.apply_ufunc(_baseline_als, da, 1e8, 0.001, input_core_dims=[[dim], [], []], output_core_dims=[[dim]],
                               vectorize=True)
    if remove_baseline:
        da = da - base_line
    ...
    prom_peak_index = xr.apply_ufunc(_index_of_largest_peak, da, prominence_factor * std, input_core_dims=[[dim], []],
                                     vectorize=True)
    peak_position = xr.apply_ufunc(_position_from_index, 1.0 * da.coords[dim], prom_peak_index,
                                   input_core_dims=[[dim], []], output_core_dims=[[]], vectorize=True)
    peak_width = xr.apply_ufunc(_width_from_index, da, prom_peak_index, input_core_dims=[[dim], []],
                                output_core_dims=[[]], vectorize=True) * dim_step
    peak_amp = da.max(dim=dim) - da.min(dim=dim) - std

    return xr.merge([peak_position.rename('position'), peak_width.rename('width'), peak_amp.rename('amplitude'),
