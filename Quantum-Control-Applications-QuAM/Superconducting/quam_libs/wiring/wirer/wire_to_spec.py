...
def allocate_dc_channels(spec: WiringSpec, instruments: Instruments):
    ...
    """
    if not (try_allocate_channels(spec, instruments, 'lf-fem') or
            try_allocate_channels(spec, instruments, 'opx+')):
...
def allocate_rf_channels(spec: WiringSpec, instruments: Instruments):
    ...
    if not try_allocate_channels(spec, instruments, 'mw-fem'):
        if not (try_allocate_channels(spec, instruments, 'octave') and
                (try_allocate_channels(spec, instruments, 'lf-fem', num=2) or
                 try_allocate_channels(spec, instruments, 'opx+', num=2))):
            raise WirerException(spec)

def try_allocate_channels(spec: WiringSpec, instruments: Instruments,
    ...
    return assign_channels_to_spec(spec, instruments, channel_types, same_con=True, same_slot=True)
