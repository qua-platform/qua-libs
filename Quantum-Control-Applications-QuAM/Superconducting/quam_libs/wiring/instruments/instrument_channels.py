...
class InstrumentChannelLfFemInput(InstrumentChannelInput): pass
class InstrumentChannelLfFemOutput(InstrumentChannelOutput): pass
class InstrumentChannelMwFemInput(InstrumentChannelInput): pass
class InstrumentChannelMwFemOutput(InstrumentChannelOutput): pass
class InstrumentChannelOpxPlusInput(InstrumentChannelInput): pass
class InstrumentChannelOpxPlusOutput(InstrumentChannelOutput): pass
class InstrumentChannelOctaveInput(InstrumentChannelInput): pass
class InstrumentChannelOctaveOutput(InstrumentChannelOutput): pass

CHANNELS_OPX_PLUS = [InstrumentChannelOpxPlusInput,
                     InstrumentChannelOpxPlusOutput]

CHANNELS_OPX_1000 = [InstrumentChannelLfFemInput,
    ...
    def check_if_already_occupied(self, channel: InstrumentChannel):
        for channel_type in self.stack:
            for existing_channel in self.stack[channel_type]:
                if (channel.con == existing_channel.con and
                        channel.slot == existing_channel.slot and
                        channel.port == existing_channel.port and
                        channel.io_type == existing_channel.io_type):
                    if channel.slot is None:
                        if type(channel) != type(existing_channel):
                            pass
                        else:
                            raise ValueError(f"{channel.io_type} channel on con{channel.con}, "
