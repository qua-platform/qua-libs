"""Analysis test for 11a_ramsey (±δ triangulation).

Uses virtual_qpu with Lindblad master equation (solver="me") to simulate
a Ramsey experiment at two symmetric detunings ±δ from the qubit
frequency.  Pure dephasing via T2 collapse operators produces the
exponential envelope that the analysis node fits.

Fitting a damped cosine to each trace and triangulating gives the
residual frequency offset Δ = (f₋ − f₊) / 2.
...
def _make_ramsey_device() -> LossDiVincenzoDevice:
    """Create a 2-qubit device with T1/T2 dephasing for Lindblad simulation."""
    params = ld_params_with_decoherence([T1_NS, T1_NS], [T2_NS, T2_NS])
    return LossDiVincenzoDevice(params=params)


@pytest.mark.analysis
def test_11a_ramsey_analysis(calibrated_pi_half_amp, analysis_runner):
    """±δ Ramsey with Lindblad dephasing and triangulated frequency."""
    device = _make_ramsey_device()
    ...
    # ── Simulate with master equation solver for physical dephasing ───────
    ...
    assert (
        0.1 < f_plus_mhz < 10.0
    ), f"f₊ should be in (0.1, 10) MHz, got {f_plus_mhz:.3f}"
    assert (
        0.1 < f_minus_mhz < 10.0
    ), f"f₋ should be in (0.1, 10) MHz, got {f_minus_mhz:.3f}"
    ...
    assert (
        np.isfinite(t2_star) and t2_star > 0
    ), f"Expected finite T2* > 0, got {t2_star}"
