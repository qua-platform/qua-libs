"""Analysis test for ``06c_PSB_search_opx_sweep_ramp_rate``.

Mirrors the structure of ``test_06b_psb_measure_duration_analysis``:
skips QUA program generation / hardware execution entirely, synthesises a
shot-by-shot IQ dataset over a ramp-duration sweep, then runs the node's
``analyse_data``, ``plot_data``, and ``update_state`` actions.  Figures and a
README are written to ``tests/analysis/artifacts/``.

The node delegates analysis to ``fit_measure_duration_raw_data`` (PCA + EM on
each ramp-duration slice).  The synthetic data places the singlet and triplet
blobs at increasing separation as ramp duration grows, so the optimal sweep
value is at the longest ramp duration.
"""
...
def _simulate_psb_ramp_sweep(
    *,
    ...
    """Generate shot-by-shot I, Q for a ramp-duration sweep.

    ``mu_T`` is linearly interpolated from ``mu_S`` toward ``mu_T_max`` as
    ramp duration increases, so blob separation (and fidelity) grows
    monotonically with sweep index — the optimum lands at the longest duration.
    """
...
def test_06c_psb_sweep_ramp_rate_analysis(minimal_quam_factory):
    ...
    ramp_min, ramp_max, ramp_step = 16, 400, 48
    sweep_values_ns = np.arange(ramp_min, ramp_max, ramp_step, dtype=float)
    num_shots = 1000
    ...
    for qp_name, r in node.results["fit_results"].items():
        ...
        assert r["sweep_name"] == "ramp_duration"
    ...
    for expected_key in (
        "fidelity_vs_ramp_duration",
        "visibility_vs_ramp_duration",
        "sweep_summary",
        "rotated_iq_density",
    ):
    ...
    assert any(
        ...
        for k in ("fidelity_vs_ramp_duration", "visibility_vs_ramp_duration", "sweep_summary")
