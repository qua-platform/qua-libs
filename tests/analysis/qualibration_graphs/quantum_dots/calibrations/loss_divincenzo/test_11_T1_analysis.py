"""Analysis test for 11_T1 (T₁ relaxation measurement).
...
def _make_t1_device() -> LossDiVincenzoDevice:
    ...
    params = ld_params_with_decoherence(
        [Q1_T1_NS, Q1_T1_NS],
        [Q1_T2_NS, Q1_T2_NS],
    )
...
def _simulate_q1_decay(device, pi_amp, tau_values_ns):
    ...
    result = simulate_sweep(
        device,
        make_t1_schedule,
        tsave=lambda idle_time, **_: jnp.array(
            [0.0, PI_PULSE_DUR + idle_time], dtype=jnp.float32
        ),
        solver="me",
...
def test_11_T1_analysis(calibrated_pi_half_amp, analysis_runner):
    ...
    assert (
        fit_q1["amplitude"] > 0.01
    ), f"q1 expected positive amplitude, got {fit_q1['amplitude']}"
    ...
    assert (
        fit_q2["amplitude"] > 0.01
    ), f"q2 expected positive amplitude, got {fit_q2['amplitude']}"
