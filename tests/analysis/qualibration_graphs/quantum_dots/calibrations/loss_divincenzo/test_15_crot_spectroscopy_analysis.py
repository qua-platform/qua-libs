"""Analysis test for 15_crot_spectroscopy.
...
# ``exchange_0_1`` carries barrier voltage in volts; J(V) follows
# ``tests/analysis/.../loss_divincenzo/virtual_ld_defaults.py`` (via ``ld_device``).
...
# CROT holds exchange on with ESR concurrent — stiffer than node-16 schedules at high V.
_SOLVER_KW = {"method": dq.method.Tsit5(max_steps=1_000_000)}


@pytest.mark.analysis
def test_15_crot_spectroscopy_analysis(
    ld_device,
    ...
    sched_example.play(
        SquarePulse(duration=CROT_PULSE_DURATION, amplitude=first_v, frequency=0.0),
        channel="exchange_0_1",
    )
    sched_example.play(
        GaussianIQPulse(
    ...
    ds_raw = build_joint_stream_analysis_ds(
        ...
        signal_per_qubit={"q1_q2": pdiff_3d},
    ...
    assert (
        fit["exchange_coupling_J"] > 0
    ), f"J should be positive, got {fit['exchange_coupling_J']}"
    assert np.isfinite(
        fit["exchange_coupling_J"]
    ), f"J should be finite, got {fit['exchange_coupling_J']}"
    assert np.isfinite(
        fit["crot_frequency_down"]
    ), f"f_down should be finite, got {fit['crot_frequency_down']}"
    assert np.isfinite(
        fit["crot_frequency_up"]
    ), f"f_up should be finite, got {fit['crot_frequency_up']}"

    j_from_splitting = abs(fit["crot_frequency_up"] - fit["crot_frequency_down"])
    assert (
        abs(j_from_splitting - fit["exchange_coupling_J"])
        < 0.3 * fit["exchange_coupling_J"]
