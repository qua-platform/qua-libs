"""Analysis test for 11c_ramsey_chevron.
...
MAX_TAU_NS = 1500
N_TAU_POINTS = 200
DETUNING_SPAN_MHZ = 4.0
DETUNING_STEP_MHZ = 0.005


@pytest.mark.analysis
def test_11c_ramsey_chevron_analysis(
    ld_device, calibrated_pi_half_amp, analysis_runner
):
    """Ramsey chevron with pre-calibrated pi/2 pulse and per-slice FFT analysis."""
    device = ld_device
    qubit_freq_ghz = device.params.qubit_freqs[0] + 0.1e-3
    ...
    ds_raw = build_joint_stream_analysis_ds(
        ...
        signal_per_qubit={q: pdiff_q1 for q in ANALYSE_QUBITS},
    )
    ...
    assert (
        np.isfinite(sigma_g) and sigma_g >= 0
    ), f"Expected finite sigma_g >= 0, got {sigma_g}"
