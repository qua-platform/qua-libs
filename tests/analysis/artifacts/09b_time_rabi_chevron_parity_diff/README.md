...
| `mcmc_num_chains` | `1` | Number of MCMC chains. Default is 1. |
| `mcmc_num_samples` | `1000` | Number of posterior samples per chain. Default is 500. |
| `mcmc_num_warmup` | `1000` | Number of NUTS warmup iterations. Default is 500. |
...
| `model_fields_set` | `{'qubits', 'mcmc_num_samples', 'simulate', 'time_step_in_ns', 'min_wait_time_in_ns', 'frequency_step_in_mhz', 'num_shots', 'frequency_span_in_mhz', 'max_wait_time_in_ns', 'mcmc_num_warmup'}` |  |
...
| `targets` | `['Q1']` |  |
| `targets_name` | `qubits` |  |
| `time_step_in_ns` | `2` | Step size for the pulse duration sweep in nanoseconds. Default is 52 ns. |
| `timeout` | `120` | Waiting time for the OPX resources to become available before giving up (in seconds). Default is 120 s. |
| `use_numpyro` | `True` | If True, use Bayesian (NumPyro NUTS) fit instead of scipy curve_fit. Default is True. |
