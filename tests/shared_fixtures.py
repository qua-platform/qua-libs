...
_local_cu_path = str(REPO_ROOT / "qualibration_graphs" / "quantum_dots" / "calibration_utils")
_cu_mod = sys.modules.get("calibration_utils")
if _cu_mod is not None and hasattr(_cu_mod, "__path__") and _local_cu_path not in list(_cu_mod.__path__):
...
def configure_qualibrate(library_root: Path) -> None:
    ...
    try:
        from qualibrate.core import config as qualibrate_config
    except Exception:
        return
    try:
