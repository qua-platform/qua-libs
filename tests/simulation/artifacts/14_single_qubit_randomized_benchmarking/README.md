...
| `gap_wait_time_in_ns` | `1024` | Initialization hold time in nanoseconds (e.g. 400 µs). Default is 400_000. |
...
| `model_fields_set` | `{'simulation_duration_ns', 'seed', 'operation_x90', 'load_data_id', 'gap_wait_time_in_ns', 'use_waveform_report', 'simulate', 'multiplexed', 'log_scale', 'timeout', 'use_state_discrimination', 'num_circuits_per_length', 'operation_x180', 'reset_wait_time', 'qubits', 'num_shots', 'delta_clifford', 'max_circuit_depth'}` |  |
...
| `targets` | `None` |  |
| `targets_name` | `qubits` |  |
| `timeout` | `60` | Waiting time for the OPX resources to become available before giving up (in seconds). Default is 120 s. |
