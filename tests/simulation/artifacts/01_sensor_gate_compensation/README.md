...
| `fit_kwargs` | `None` | Extra keyword arguments forwarded to the chosen fitting function.
For ``bayesian_cp``: ``{"hazard": 1/30, "cp_threshold": 0.3}``. |
| `fit_method` | `bayesian_cp` | Fitting method: ``"bayesian_cp"`` (robust to charge transitions via
