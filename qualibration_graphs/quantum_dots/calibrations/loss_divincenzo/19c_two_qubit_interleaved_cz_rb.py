"""Two-qubit interleaved (CZ) randomized benchmarking calibration node.
...
from calibration_utils.two_qubit_rb.rb_utils import InterleavedRB
...
Interleaved randomized benchmarking isolates the error for a specific target get (here CZ) by comparing the survival probability
to an exponential decay as a function of circuit depth. This gives an estimate of the specific gate error rate for
...
node = QualibrationNode[Parameters, Quam](
    name="19c_two_qubit_interleaved_cz_rb",  # Name should be unique
...
def plot_data(node: QualibrationNode[Parameters, Quam]):
    ...
    for qp in qubit_pairs:
        ...
        macro_fidelity = qubit_pair.gate_fidelity.get(node.parameters.operation, {})
        standard_rb_alpha = macro_fidelity.get("StandardRB_alpha")

        if standard_rb_alpha is None:
            raise RuntimeError(
                f"StandardRB_alpha not found for {qp.name}. "
                "Run 19_two_qubit_standard_rb first to populate the "
                "standard RB reference data needed for interleaved RB."
            )
        ...
        standard_rb_load_id = macro_fidelity.get("StandardRB_load_id")
        if standard_rb_load_id is not None:
            try:
                ...
                std_rb_ds = std_rb_data["ds_raw"]

                std_rb_result = RBResult(
                    circuit_depths=list(std_rb_ds.circuit_depth.values),
                    num_repeats=len(std_rb_ds.repeat),
                    num_averages=len(std_rb_ds.average),
                    state=std_rb_ds.state.sel(qubit_pair=qp.name).data,
                )

                A_std, alpha_std, B_std = std_rb_result.fit_exponential()
                ax = fig.axes[0]
                std_circuit_depths = list(std_rb_ds.circuit_depth.values)
                ax.plot(
                    std_circuit_depths, std_rb_result.get_decay_curve(), "s",
                    color="green", alpha=0.5, label="StandardRB Data",
                )
                smooth_x = np.linspace(std_circuit_depths[0], std_circuit_depths[-1], 100)
                ax.plot(
                    smooth_x, rb_decay_curve(smooth_x, A_std, alpha_std, B_std),
                    color="green", linestyle="--",
                    label=f"StandardRB Fit (alpha={alpha_std:.4f})",
                )
                ax.legend(framealpha=0)
            except Exception as e:
                node.log(f"Could not overlay StandardRB data: {e}")
...
def update_state(node: QualibrationNode[Parameters, Quam]):
    ...
    with node.record_state_updates():
        for qp in node.namespace["qubit_pairs"]:
            ...
            # The macro's own `fidelity` attribute is typed Optional[float], so it
            # must not hold a dict of metrics.
            qubit_pair = node.machine.qubit_pairs[qp.name]
            operation = node.parameters.operation
            gate_metrics = qubit_pair.gate_fidelity.setdefault(operation, {})
            gate_metrics["InterleavedRB"] = node.results["fit_results"][qp.name]["fidelity"]
            gate_metrics["InterleavedRB_alpha"] = node.results["fit_results"][qp.name]["alpha"]
