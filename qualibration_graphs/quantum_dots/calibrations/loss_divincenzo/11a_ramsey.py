...
Results (``node.results["fit_results"][qubit]``):
    - ``success``: whether both Ramsey traces were fitted successfully.
    - ``freq_offset`` [Hz]: triangulated residual frequency offset.
    - ``t2_star`` [ns]: fitted Ramsey dephasing time.
    - ``decay_rate`` [1 / ns]: fitted Ramsey envelope decay rate.
    - ``freq_plus`` [Hz]: fitted oscillation frequency for the ``+δ`` trace.
    - ``freq_minus`` [Hz]: fitted oscillation frequency for the ``−δ`` trace.

Figures (``node.results["figures"]``):
    - ``"raw_data_with_fit"``: Ramsey traces at ``+δ`` and ``−δ`` with damped-cosine fits.
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    """Build the 2D detuning × idle-time Ramsey sweep and the QUA pulse sequence."""
    ...
    # Extract the detuning value, and construct a small array of ±detuning
    ...
    with program() as node.namespace["qua_program"]:
        ...
        for i, qubit in enumerate(qubits):
            ...
            with for_(n, 0, n < n_avg, n + 1):
                ...
                # ── MIDDLE LOOP: repeat the Ramsey sweep for +δ and −δ ───────────
...
def generate_simulated_data(node: QualibrationNode[Parameters, Quam]):
    """Generate simulated Ramsey data so the full analysis pipeline can run without hardware."""
