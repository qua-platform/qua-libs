...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    with program() as node.namespace["qua_program"]:
        ...
        with stream_processing():
            ...
            n_dfs = len(dfs)
            for i, qubit in enumerate(qubits):
                p1_st[qubit.name].buffer(n_dfs).average().save(f"pre{i + 1}")
                p2_st[qubit.name].buffer(n_dfs).average().save(f"post{i + 1}")
                pdiff_st[qubit.name].buffer(n_dfs).average().save(f"pdiff{i + 1}")
