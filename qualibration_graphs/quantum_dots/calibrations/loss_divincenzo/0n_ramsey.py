...
from calibration_utils.common_utils.qubit_readout_helper import get_qubits_batched_by_readout
from qualibration_libs.parameters import get_idle_times_in_clock_cycles
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    node.namespace["qubits"] = qubits = get_qubits_batched_by_readout(node)
    ...
    node.namespace["sweep_axes"] = {
        "qubit": xr.DataArray([q.id for q in qubits]),
                ...
                with for_each_(idle_time, idle_times):
                    with for_(*from_array(detuning_sign, detuning_signs)):
                        ...
                        for i, qubit in batched_qubits.items():
                            ...
                            # with strict_timing_():
                            qubit.xy_channel.play("x90")
                            qubit.xy_channel.frame_rotation_2pi(virtual_detuning_phases[i])
                            qubit.xy_channel.wait(idle_time)
                            qubit.xy_channel.play("x90")
                        ...
                        for i, qubit in batched_qubits.items():
                            qubit.sensor_dots[0].readout_resonator.measure("readout", qua_vars=(I[i], Q[i]))
                            save(I[i], I_st[i])
                            save(Q[i], Q_st[i])
                        align()
