...
from calibration_utils.opx_external_2d_sweep import Parameters, get_voltage_arrays
from calibration_utils.opx_external_2d_sweep import (
    plot_raw_amplitude, 
    plot_raw_phase
)
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    dot_pair_name = node.machine.find_quantum_dot_pair(node.parameters.x_axis_name, node.parameters.y_axis_name)
    sensor_list = node.machine.quantum_dot_pairs[dot_pair_name].sensor_dots
    node.namespace["sensors"] = sensors = _make_batchable_list_from_multiplexed(sensor_list, False)
                    ...
                    with for_(*from_array(x, x_volts)): 
                        with for_(*from_array(y, y_volts)):
                            with seq.simultaneous():
                                x_obj.go_to_voltages({x_obj.name: x}, duration = dwell_time)
                                y_obj.go_to_voltages({y_obj.name: y}, duration = dwell_time)
                            align()
