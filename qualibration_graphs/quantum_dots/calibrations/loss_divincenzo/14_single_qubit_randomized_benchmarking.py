...
Prerequisites:
    - Having calibrated the mixer or the Octave and the time of flight (nodes 01a and 01b).
    - Having calibrated the sensor dots (and relevant resonators) associated to the measured qubits (nodes 2a, b and 3).
    - Having calibrated initialization, operation and PSB measurement points (nodes 4 and 5 with VideoMode).
    - Having calibrated the qubit parameters precisely (nodes 08a_power_rabi.py and 10a_ramsey.py).
    - (optional) Having optimized the readout parameters (nodes 08a, 08b).
