...
from calibration_utils.sensor_gate_sweep_dac import Parameters
from calibration_utils.sensor_gate_sweep_dac import (
    process_raw_dataset,
    fit_raw_data,
    log_fitted_results,
    generate_simulated_dataset,
)
from calibration_utils.sensor_gate_sweep_dac import plot_raw_phase, plot_amplitude_with_fit
from calibration_utils.charge_stability_dac import prepare_dacs_and_gatemanager, close_dacs
...
# %% {Node initialisation}
description = """
        CHARGE SENSOR GATE SWEEP with the DAC
...
node = QualibrationNode[Parameters, Quam](
    name="03c_sensor_gate_sweep_dac", description=description, parameters=Parameters()
)
...
def custom_param(node: QualibrationNode[Parameters, Quam]):
    ...
    node.parameters.offset_span = 0.02
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    dot_pair = [node.machine.get_component(qp).quantum_dot_pair for qp in node.parameters.qubit_pair][0]
...
def execute_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    # Prepare DAC
    dac1, dac2, _, gates = prepare_dacs_and_gatemanager()
