...
The QUA program is divided into three sections:
    ...
    3) measure the state of the qubit using RF reflectometry.
...
Results (``node.results["fit_results"][qubit]``):
    - ``success``: whether the exponential-decay fit passed the node criteria.
    - ``T1`` [ns]: fitted longitudinal relaxation time.
    - ``amplitude``: fitted decay amplitude.
    - ``offset``: fitted asymptotic baseline.
    - ``decay_rate`` [1 / ns]: fitted decay rate ``1 / T1``.

Figures (``node.results["figures"]``):
    - ``"raw_data_with_fit"``: state vs idle time with exponential-fit overlay for each qubit.
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    """Build the 1D idle-time sweep and the QUA pulse sequence."""
    ...
    n_avg = node.parameters.num_shots  # repetitions averaged at each idle-time point
    ...
    with program() as node.namespace["qua_program"]:
        ...
        with stream_processing():
            ...
            for i in range(num_qubits):
                # Each save() is one idle-time point.
                # .buffer(len(tau_values)) : group points along the idle-time axis
...
def generate_simulated_data(node: QualibrationNode[Parameters, Quam]):
    """Generate synthetic T1 data so the full analysis pipeline can run without hardware."""
