...
**`state_macros/`**: Contains reusable custom initialize and measure macro implementations. Each macro can expose a `Parameters` class so that `my_macros.py` can surface its configurable fields in Qualibrate nodes.
The underlying macro infrastructure in `quam-builder` lives in the
