...
[tutorial README](state_macros/README.md). Each macro declares its configurable attributes directly on the
macro dataclass, and `CustomMacro.Parameters` automatically exposes those fields as an optional
Qualibrate-compatible parameter model. The underlying macro infrastructure in `quam-builder` lives in the
[`operations` package](https://github.com/qua-platform/quam-builder/tree/main/quam_builder/architecture/quantum_dots/operations).
...
- Declare configurable fields directly on the macro dataclass. `CustomMacro.Parameters` will derive the node-facing parameter model from those fields automatically.
