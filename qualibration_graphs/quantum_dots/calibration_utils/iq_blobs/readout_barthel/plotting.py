    ...
    *,
    ...
    if posterior == "ppd_mean":
        ...
        ax.text(
            0.02,
            0.98,
            f"w_S={1-pT_m:.2f}  w_T(no)={pT_m*p_no:.2f}  w_T(dec)={pT_m*(1-p_no):.2f}\n"
        ...
        # point-estimate plug-in
        mu_S = float(_agg("mu_S"))
        mu_T = float(_agg("mu_T"))
        sigma = float(_agg("sigma"))
        pT = float(_agg("pT"))
        T1 = float(_agg("T1"))
...
def plot_iq_with_pca_and_threshold(
    ...
    *,
    ...
    if pca_kwargs is None:
        pca_kwargs = dict(color="C2", lw=2)
    if thr_kwargs is None:
