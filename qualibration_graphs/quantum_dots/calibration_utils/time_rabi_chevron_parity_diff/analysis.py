"""Chevron fit via generalized Rabi formula P(t,Δ)=A·(Ω/Ω_R)²·sin²(Ω_R·t/2)+offset. Extracts f_res, t_π."""
...
def _rabi_chevron_model(
    ...
    """Rabi model P(t,Δ)=A·(Ω/Ω_R)²·sin²(Ω_R·t/2)+offset. params=(f_res, omega, A, offset). Returns flat 1D."""
    f_res, omega, amplitude, offset = params
    # Δ in rad/ns: 2π·(f_drive - f_res)·1e-9
    delta_rad_per_ns = 2.0 * np.pi * (freq_hz - f_res) * 1e-9
    # Build 2D grids
    f_2d, t_2d = np.meshgrid(freq_hz, duration_ns, indexing="ij")
    delta_2d = 2.0 * np.pi * (f_2d - f_res) * 1e-9
    ...
    return (amplitude * visibility * np.sin(phase) ** 2 + offset).ravel()
...
def _fit_chevron_single_qubit(
    ...
    f_flat = np.tile(freqs_hz, len(durations_ns))
    t_flat = np.repeat(durations_ns, len(freqs_hz))
    ...
    bounds = (
        [f_min - 1e6, omega_min, 0.0, -0.1],
        [f_max + 1e6, omega_max, 2.0, 1.1],
    )

    def _model_flat(_x: np.ndarray, f_res: float, om: float, A: float, off: float) -> np.ndarray:
        """Wrapper for curve_fit; _x unused, uses freqs_hz/durations_ns from closure."""
        return _rabi_chevron_model(
            np.array([f_res, om, A, off]),
            freqs_hz,
            durations_ns,
        )
    ...
    f_res, omega, amplitude, offset = popt
    t_pi_ns = np.pi / omega
...
def fit_raw_data(ds: xr.Dataset, node: QualibrationNode) -> Tuple[xr.Dataset, Dict[str, Dict[str, Any]]]:
    ...
    for qname in qubit_names:
        ...
        fit_results[qname] = asdict(fp)
        if "optimal_frequency_std" in result:
            fit_results[qname]["optimal_frequency_std"] = result["optimal_frequency_std"]
            fit_results[qname]["optimal_duration_std"] = result["optimal_duration_std"]
...
def log_fitted_results(
    ...
    for qname, r in fit_results.items():
        ...
        msg = f"Results for {qname}: " f"f_res={f_res:.4f} GHz, " f"t_π={t_pi:.0f} ns, " f"success={success}"
        log_callable(msg)
