"""Data analysis utilities for randomized benchmarking experiments.

This module provides classes for analyzing and visualizing RB experiment results,
including fitting exponential decay curves and calculating gate fidelities.
"""
...
class RBResult:  # pylint: disable=too-many-instance-attributes
    """
    ...
    def plot_hist(self, n_cols=3):
        ...
        """
        n_cols = min(n_cols, len(self.circuit_depths))
    ...
    def plot(self):
        ...
        """
        recovery_probability = (self.data.state == 0).sum(
            ("repeat", "average")
    ...
    def plot_with_fidelity(self):
        ...
        label = (
            ...
            if isinstance(self, InterleavedRBResult)
                ...
                f"Single 2Q Gate Fidelity = {self.average_gate_fidelity * 100:.2f}%"
            )
        )
    ...
    def plot_two_qubit_state_distribution(self):
        ...
        plt.plot(
            ...
            .mean(dim="average")
            .mean(dim="repeat")
            .data,  # pylint: disable=use-implicit-booleaness-not-comparison-to-zero
            label=r"$|00\rangle$",
            marker=".",
            color="c",
            linewidth=3,
