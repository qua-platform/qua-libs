...
Provides:
- declare_streams  – declare QUA variables/streams inside a program() block
- save_measurement – save one shot's parity outcome to the right stream
- buffer_streams   – wire up stream_processing() for one item
...
def declare_streams(node, items, stream_fn: Optional[Callable] = None):
    ...
    Args:
        node: QualibrationNode whose ``parameters.parity_measurement``
    ...
    Returns:
        ...
        * ``p0`` – QUA ``int`` variable for the pre-sequence measurement, or
          ``None`` when ``parity_measurement`` is ``False``.
        * ``streams`` – dict mapping stream-key → ``{item.name: stream}``.
          Keys are ``"p0_p0"``, ``"p0_p1"``, ``"p1_p0"``, ``"p1_p1"`` when
          ``parity_measurement`` is ``True``, or ``"p"`` otherwise.

    Example:
        >>> with program() as node.namespace["qua_program"]:
        ...     p1, p0, streams = declare_streams(node, qubits)
        ...     with for_(shot, 0, shot < num_shots, shot + 1):
        ...         for qubit in qubits:
        ...             assign(p1, Cast.to_int(qubit.measure()))
        ...             save_measurement(node, qubit.name, p0, p1, streams)
        ...     with stream_processing():
        ...         for qubit in qubits:
        ...             buffer_streams(node, qubit.name, streams, num_sweep_points)
    """
...
def save_measurement(node, name: str, p0, p1, streams: dict) -> None:
    ...
    Args:
        ...
        p0: QUA variable holding the pre-sequence measurement result (ignored
            when ``parity_measurement`` is ``False``).
        p1: QUA variable holding the post-sequence measurement result.
        streams: The ``streams`` dict returned by :func:`declare_streams`.
...
def get_parity_item_names(
    ...
    *,
    ...
    :func:`process_streams`.  Raw joint streams and single-shot streams
...
def process_streams(
    ...
    *,
    ...
    Args:
        ...
        parity_measurement: Typically ``node.parameters.parity_measurement``
