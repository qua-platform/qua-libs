...
1. For each drive-detuning slice, compute the FFT and fit the spectral
   peak with a Gaussian (natural for a Gaussian pulse envelope), falling
   back to Lorentzian if needed.
2. Find f_res from the amplitude-weighted centroid of per-slice peak
   positions.
3. Extract f_Ω from a marginalised spectrum (average of the top-K
   highest-amplitude FFT slices near resonance).
4. Derive t_π = π/Ω and the analytical Rabi hyperbola for plotting.
"""
...
FFT_FREQ_MAX = 0.03  # t_π min ~17 ns; restricts to plausible Rabi range
