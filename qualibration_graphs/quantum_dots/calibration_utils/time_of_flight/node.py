...
def patch_readout_pulse_params(
    resonators: List[ReadoutResonatorSingle],
    ...
    for resonator in resonators:
        ...
        with tracked_updates(
            ...
            resonator.operations["readout"].length = (
                node_parameters.readout_length_in_ns
            )
            resonator.operations["readout"].amplitude = (
                node_parameters.readout_amplitude_in_v
            )
            if node_parameters.intermediate_frequency_in_mhz is not None:
                resonator.intermediate_frequency = int(
                    node_parameters.intermediate_frequency_in_mhz * 1e6
                )
