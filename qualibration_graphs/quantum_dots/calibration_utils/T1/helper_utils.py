...
def _find_readouts(qubit: LDQubit):
    """Returns the readout resonator object associated with the qubit's readout macro"""
    machine = qubit.machine

    qd_pair_name = machine.find_quantum_dot_pair(qubit.quantum_dot.name, qubit.preferred_readout_quantum_dot)
...
def elements_list(qubit: LDQubit) -> List[str]:
