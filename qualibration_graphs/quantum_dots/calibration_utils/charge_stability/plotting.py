...
def plot_peak_locations(
    ...
    """
    Plot the detected peak locations on the charge stability map.
    ...
    sensor_data : xr.Dataset
        Dataset containing the sensor's I and Q quadrature data.
    fit_params : dict
        Dictionary containing fit parameters with peak location arrays.
    sensor_id : str
        The sensor ID for the title.
    ...
    # Get the arrays from fit_params and convert to numpy if needed
    col_peaks = np.array(fit_params['col_peaks'])
    row_peaks = np.array(fit_params['row_peaks'])
    col_peaks2 = np.array(fit_params['col_peaks2'])
    row_peaks2 = np.array(fit_params['row_peaks2'])

    # Scatter plot peak locations
    if len(col_peaks) > 0:
        for i in range(min(3, len(col_peaks))):
            if i < len(col_peaks):
                ax.scatter(col_peaks[i], row_peaks[i], label=f'Vertical peak {i+1}', alpha=0.75)

    if len(col_peaks2) > 0:
        for i in range(min(3, len(col_peaks2))):
            if i < len(row_peaks2):
                ax.scatter(row_peaks2[i], col_peaks2[i], label=f'Horizontal peak {i+1}', alpha=0.75)

    ax.set_xlabel("Column index")
    ax.set_ylabel("Row index")
    ax.set_title(f"{sensor_id} - Peak locations")
    ax.legend()
...
def plot_gap_shoulders(
        sensor_data: xr.Dataset,
        fit_params: dict,
        sensor_id: str
) -> Figure:
    """
    Plot gap shoulders for detected transitions.
    ...
    sensor_data : xr.Dataset
        Dataset containing the sensor's I and Q quadrature data.
    fit_params : dict
        Dictionary containing fit parameters with 'results' and 'results2'.
    sensor_id : str
        The sensor ID for the title.
    ...
    # Plot horizontal transitions
    ax1.imshow(amplitude, origin="lower")
    row_peaks2 = np.array(fit_params['row_peaks2'])
    col_peaks2 = np.array(fit_params['col_peaks2'])
    results2 = fit_params['results2']

    if len(results2) > 0:
        for r, c, res2 in zip(row_peaks2, col_peaks2, results2):
            x = r
            y = c
            ax1.plot(x, y, 'o', ms=3, alpha=0.5)
            ax1.plot(x, res2['y_smooth'], '-', lw=2)
            ax1.scatter(res2['xL'], res2['yL'], s=120, marker='*', c='gold', edgecolor='k', zorder=5)
            ax1.scatter(res2['xR'], res2['yR'], s=120, marker='*', c='gold', edgecolor='k', zorder=5)

    ax1.set_title(f'{sensor_id} - Horizontal Transitions')

    # Plot vertical transitions
    ax2.imshow(amplitude, origin="lower")
    row_peaks = np.array(fit_params['row_peaks'])
    col_peaks = np.array(fit_params['col_peaks'])
    results = fit_params['results']

    if len(results) > 0:
        for r, c, res in zip(row_peaks, col_peaks, results):
            x = r
            y = c
            ax2.plot(y, x, 'o', ms=3, alpha=0.5)
            ax2.plot(res['y_smooth'], x, '-', lw=2)
            ax2.scatter(res['yL'], res['xL'], s=120, marker='*', c='gold', edgecolor='k', zorder=5)
            ax2.scatter(res['yR'], res['xR'], s=120, marker='*', c='gold', edgecolor='k', zorder=5)

    ax2.set_title(f'{sensor_id} - Vertical Transitions')

    fig.tight_layout()
    return fig


def plot_charge_state_boundaries(
        sensor_data: xr.Dataset,
        fit_params: dict,
        sensor_id: str,
        state: tuple = (0, 0)
    ...
    """
    Plot the charge state boundaries for a specific state.
    ...
    sensor_id : str
        The sensor ID for the title.
    state : tuple
        The (row, col) state index to highlight.
    ...
    fig, ax = plt.subplots(figsize=(8, 6))
    ax.imshow(amplitude, origin="lower")

    results = fit_params['results']
    results2 = fit_params['results2']

    if len(results2) > state[0] and len(results) > state[1]:
        data0 = results2[state[0]]
        data1 = results[state[1]]

        # Plot horizontal state boundaries
        ax.scatter(
            data0['xL'][state[1]], data0['yL'][state[1]],
            s=200, marker='*', c='gold', edgecolor='k', zorder=5, label='Horizontal Left'
        )
        ax.scatter(
            data0['xR'][state[1]], data0['yR'][state[1]],
            s=200, marker='*', c='gold', edgecolor='k', zorder=5, label='Horizontal Right'
        )

        # Plot vertical state boundaries
        ax.scatter(
            data1['yL'][state[0]], data1['xL'][state[0]],
            s=200, marker='*', c='red', edgecolor='k', zorder=5, label='Vertical Left'
        )
        ax.scatter(
            data1['yR'][state[0]], data1['xR'][state[0]],
            s=200, marker='*', c='red', edgecolor='k', zorder=5, label='Vertical Right'
        )

    ax.set_xticks([])
    ax.set_yticks([])
    ax.set_title(f'{sensor_id} - State {state} Boundaries')
    ax.legend()
