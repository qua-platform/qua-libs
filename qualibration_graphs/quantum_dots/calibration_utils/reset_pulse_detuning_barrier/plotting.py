...
def _plot_heatmap(fig, ax, x, y, z, title, cbar_label, cmap, vmin=None, vmax=None):
    ...
    ax.set_ylabel("Barrier gate voltage")
    ax.set_title(title)
...
def plot_2d_summary(
    ...
    fig_init.suptitle(
        "Initialised maps (averaged over pulse/no-pulse)"
        if plot_init_i
        else "Initialised state maps (averaged over pulse/no-pulse)"
    )
    ...
    for idx, q_name in enumerate(qubit_names):
        _plot_heatmap(
            ...
            detuning,
            barrier,
            map_results[q_name]["state_no"],
            f"{q_name} — state (no pulse)",
            "State",
            "RdBu_r",
            0,
            1,
        ...
        _plot_heatmap(
            ...
            detuning,
            barrier,
            map_results[q_name]["state_yes"],
            f"{q_name} — state (with pulse)",
            "State",
            "RdBu_r",
            0,
            1,
        ...
        _plot_heatmap(
            ...
            detuning,
            barrier,
            map_results[q_name]["i_yes"],
            f"{q_name} — I (with pulse)",
            "I",
            "viridis",
        )
    ...
    for idx, q_name in enumerate(qubit_names):
        ...
        if q_name in fit_results and fit_results[q_name].get("success", False):
            ...
            axes_diff[0, idx].plot(
                fit["optimal_detuning"],
                fit["optimal_barrier_gate_voltage"],
                "k*",
                markersize=14,
