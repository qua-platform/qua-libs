...
|0>, |1>, |2> populations vs time and detuning (imshow).
...
def omega_from_phi(EJ_small: jnp.ndarray, EJ_large: jnp.ndarray, EC: jnp.ndarray, phi: jnp.ndarray) -> jnp.ndarray:
    EJ = jnp.sqrt(EJ_small ** 2 + EJ_large ** 2 + 2.0 * EJ_small * EJ_large * jnp.cos(phi))
    return jnp.sqrt(8.0 * EJ * EC) - EC


def run_rabi_debug_sweep(
    params: dict[str, object] = DEBUG_PARAMS,
    ...
    # Bias the coupler away from qubits; coupling remains weak via g_couplings.
        ...
        device = SuperconductingDevice(
            ...
            g_couplings=params["g_couplings"],
            g_direct=params.get("g_direct", (0.0,)),
            ref_qubit_freqs=qubit_refs,
            ref_coupler_freqs=coupler_refs,
            ...
            coupler_phi_ext=coupler_phi_ext,
        )
    ...
    # Projectors onto |0>, |1>, |2> of qubit 0
    proj_ops = [
        embed_op(dq.proj(dq.basis(levels, k)), which=0, n_modes=device.n_modes, levels=levels)
    ...
    for d in detunings:
        ...
        pops = jnp.real(res.expects)
        if pops.shape[0] == levels and pops.shape[1] == tsave.size:
            pops = pops.T
        all_pops.append(pops)

    populations = jnp.stack(all_pops, axis=0)
    return detunings, tsave, populations


if __name__ == "__main__":
    detunings, t, pops = run_rabi_debug_sweep(params=PAPER_PARAMS_SYMMETRIC)
    ...
    fig.suptitle("Q0 Rabi Populations vs Time and Detuning")
    fig.tight_layout()

    data_dir = Path(__file__).resolve().parent / "data"
    data_dir.mkdir(parents=True, exist_ok=True)
    out_path = data_dir / "rabi_debug_detuning.png"
