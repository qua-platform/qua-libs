...
def resolve_resspec_params(node: QualibrationNode, target: str) -> bool:
    ...
    if fit.get("success"):
        return {
            "frequency_span_in_mhz": curr_span* 0.6, 
            "frequency_step_in_mhz": curr_step*0.6
        }
...
def resolve_qspec_params_advanced(node: QualibrationNode, target: str) -> bool:
    ...
    if not qubit_fit or qubit_fit.get("success"):
        return {"operation_amplitude_factor": max_power_to_rerun,
    ...
    if qubit_fit.get("peak_snr",float("inf")) and qubit_fit.get("r2", float("inf"))<snr_threshold:
    ...
    if len(return_dict)<1:
        return_dict= {"operation_amplitude_factor": min(max_power_to_rerun,amp_factor),
...
def validate_qspec_fwhm(node: QualibrationNode, target: str) -> bool:
    ...
    if not fit.get("success"):
        node.log(
        f"Fit was not successful the loop will rerun."
        )
        return True
