...
from calibration_utils.chevron_cz.parameters import moving_qubit
...
def process_raw_dataset(ds: xr.Dataset, node: QualibrationNode):
    ...
    """
    def detuning(qp, amp):
        return -((amp * node.namespace["pulse_amplitudes"][qp.name]) ** 2) * moving_qubit(qp).freq_vs_flux_01_quad_term
    ...
    ds = ds.assign_coords(
        {
            "quad_term_moving": (
                ["qubit_pair"],
                np.array([moving_qubit(qp).freq_vs_flux_01_quad_term for qp in qubit_pairs]),
            )
        }
    )
