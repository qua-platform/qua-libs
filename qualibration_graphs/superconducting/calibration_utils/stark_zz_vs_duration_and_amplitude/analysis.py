...
def log_fitted_results(fit_results: Dict, log_callable=None):
    ...
    for qp in fit_results.keys():
        ...
        s_amp_scaling = (
            f"\tmax zz_coeff at amp_scaling={fit_results[qp]['best_amp_scaling']}\n"
        )
        ...
        if fit_results[qp]["success"]:
            ...
            s_qubit_pair += " FAIL!\n"
        log_callable(s_qubit_pair + s_amp_scaling + s_zz_coeff + s_detuning)
...
def _extract_relevant_fit_parameters(
    ...
    # Best amp_scaling per qubit_pair
    best_amp_scaling = zz_coeff.idxmax(dim="amp_scaling").rename("best_amp_scaling")
    best_zz_coeff = zz_coeff.max(dim="amp_scaling").rename("best_zz_coeff")
    ...
    def _sel_best(da, qp, *, control_state):
        amp_scaling = best_amp_scaling.sel(qubit_pair=qp).item()
        return (
            da.sel(qubit_pair=qp, control_target="t", control_state=control_state)
            .sel(amp_scaling=amp_scaling, method="nearest")
            .squeeze(drop=True)
            .item()
        )
