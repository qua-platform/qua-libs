...
def plot_raw_data_with_fit(
    ...
    for qp in qubit_pairs:
        ...
        correction_target_terms = ds_pair.correction_target_term.data
        control_targets = ds_pair.control_target.data
        control_states = ds_pair.control_state.data

        fig, axes = plt.subplots(2, 2, figsize=(8, 6), sharex=True, sharey=True)

        for row, tt in enumerate(correction_target_terms):

            for col, ct in enumerate(control_targets):
                ax = axes[row, col]

                for st in control_states:
                    y = ds_pair.sel(correction_target_term=tt, control_target=ct, control_state=st)[val].data
