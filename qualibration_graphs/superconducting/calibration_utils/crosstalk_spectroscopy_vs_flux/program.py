...
def get_flux_detuning_in_v(node_parameters, target_qubit) -> float:
    ...
    if node_parameters.flux_detuning_mode == "manual":
        ...
        flux_detuning = get_qubit_flux_detuning_for_one_percent_tangent_error_on_parabola(
            node_parameters.aggressor_flux_offset_span_in_v, node_parameters.expected_crosstalk
        )

    elif node_parameters.flux_detuning_mode == "auto_fill_sweep_window":
...
def get_target_slope_from_parameter_ranges(parameters) -> float:
    """Target df/dphi slope that fills the aggressor flux vs frequency sweep window."""
    freq_span = parameters.aggressor_qubit_frequency_span * 1e6
    flux_span = parameters.aggressor_flux_offset_span_in_v
