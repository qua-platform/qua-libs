...
def plot_raw_data_with_fit(ds_fit: xr.Dataset, qubit_pairs: BatchableList) -> plt.Figure:
    """Plot the JAZZ-N data and sinc fit per qubit pair.

    For each qubit pair we show two stacked panels:

    * Top: ``state_target`` (target |1> population) as a 2D map versus the
    ...
    * Bottom: the averaged-over-N curve ``p_avg(amp)`` together with the
    ...
    for i, qp in enumerate(qubit_pairs):
        ...
        # --- Top: 2D heatmap of P_|1> ---
        p_map = fr["state_target"].transpose("N", "amp")
        ...
        ax_map.set_ylabel("Echo count N = 4k + 1")
        ax_map.legend(loc="upper right", fontsize=8)
        cbar = fig.colorbar(pcm, ax=ax_map, shrink=0.85)
        cbar.set_label("$P_{|1\\rangle}$ of target")

        # --- Bottom: averaged P_|1> with sinc fit ---
        if "p_avg" in fr.data_vars:
            ax_avg.plot(amps_scale, fr["p_avg"].values, "o", ms=3, color="C0", label=r"$\langle P_{|1\rangle}\rangle_N$")
        ...
        ax_avg.set_ylabel(r"$\langle P_{|1\rangle}\rangle_N$")
    ...
    fig.suptitle("JAZZ-N CZ amplitude calibration")
