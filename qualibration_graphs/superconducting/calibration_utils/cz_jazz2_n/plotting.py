...
def plot_raw_data_with_fit(
    ...
    """Plot the JAZZ2-N data and sinc fit per qubit pair on chip-topology grids.

    For each qubit pair we show two separate figures laid out by chip topology:

    * ``"map"``: joint ``p`` (P_|00>) as a 2D map versus repetition count N = 2k
      and amplitude scale, with the fitted optimal amplitude drawn as a vertical line.
    * ``"avg"``: the averaged-over-N curve ``p_avg(amp)`` together with the
    ...
    ----------
    ds_fit : xr.Dataset
        Fit dataset containing ``p``, ``p_avg``, ``sinc_fit``,
...
def plot_individual_map_with_fit(ax: Axes, ds_fit: xr.Dataset, qp_name: str) -> None:
    ...
    ax.figure.colorbar(pcm, ax=ax, shrink=0.85).set_label("$P_{|00\\rangle}$")
