...
def plot_moving_qubit_populations(
    ...
    ``cz_iswap_flux_bootstrap.parameters.moving_qubit`` which reads the
    ``qubit_pair.moving_qubit`` attribute, so this works for both
    control-as-moving and target-as-moving architectures.
