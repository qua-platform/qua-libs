...
def log_fitted_results(fit_results: Dict, log_callable=None):
    ...
    for qp in fit_results.keys():
        ...
        s_phase = (
            f"\tmax zz_coeff at relative_phase={fit_results[qp]['best_relative_phase']}\n"
        )
        ...
        if fit_results[qp]["success"]:
            ...
            s_qubit_pair += " FAIL!\n"
        log_callable(s_qubit_pair + s_phase + s_zz_coeff + s_detuning)
...
def _extract_relevant_fit_parameters(
    ...
    # Best phase per qubit_pair
    best_phase = zz_coeff.idxmax(dim="relative_phase").rename("best_phase")
    best_zz_coeff = zz_coeff.max(dim="relative_phase").rename("best_zz_coeff")
    ...
    def _sel_best(da, qp, *, control_state):
        phase = best_phase.sel(qubit_pair=qp).item()
        return (
            da.sel(qubit_pair=qp, control_target="t", control_state=control_state)
            .sel(relative_phase=phase, method="nearest")
            .squeeze(drop=True)
            .item()
        )
    ...
    fit_results: Dict[str, FitParameters] = {
        qp: FitParameters(
            best_relative_phase=best_phase.sel(qubit_pair=qp).item(),
            zz_coeff=1e9 * best_zz_coeff.sel(qubit_pair=qp).item(),
            detuning_target_qc0=1e9 * _sel_best(fitted_frequency, qp, control_state=0),
            detuning_target_qc1=1e9 * _sel_best(fitted_frequency, qp, control_state=1),
            # qc0 should use control_state=0
            T2_modified_echo_target_qc0=1e-9
                * _sel_best(T2_modified_echo, qp, control_state=0),
            T2_modified_echo_error_target_qc0=1e-9
                * _sel_best(T2_modified_echo_error, qp, control_state=0),
            # qc1 should use control_state=1
            T2_modified_echo_target_qc1=1e-9
                * _sel_best(T2_modified_echo, qp, control_state=1),
            T2_modified_echo_error_target_qc1=1e-9
                * _sel_best(T2_modified_echo_error, qp, control_state=1),
            success=bool(
                success_criteria.sel(qubit_pair=qp, control_target="t")
                .sel(
                    relative_phase=best_phase.sel(qubit_pair=qp).item(),
                    method="nearest",
                )
