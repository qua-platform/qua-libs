...
def fit_fir_data(ds_fit: xr.Dataset, node) -> dict:
    ...
    Pipeline:
      ...
      3. Auto-tune (GCV / L-curve, selected by AIC) a forward FIR up to
    ...
    for dim_name in dim_names:
        ...
        # Noise-floor triangulation (sigma_A tail-std, sigma_B first-diff, sigma_C
        # fit-implied). Run on the 1 GS/s raw grid: at 2 GS/s the cubic
        # interpolation correlates adjacent samples and biases sigma_B downward
        # (false-positive WARN). sigma_C lives in the 2 GS/s fit domain — close
        # enough in magnitude for the loose 1.5 ratio test.
        sigma_C = None
        ...
        node.log(
            f"  {dim_name}: FIR done (auto) — L={auto_info['L']}, "
            f"NRMS={auto_info['forward_nrms']:.3e}, "
        ...
        if noise_info["status"] == "warn_tail":
            node.log(f"  {dim_name}: tail may not be settled (sigma_A >> sigma_B); try a longer cryoscope_len.")
