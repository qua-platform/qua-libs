...
def optimize_fir_parameters(
    ...
    if verbose and best_params is not None:
        print(
            f"  Best: L={best_params['L']}, lam1={best_params['lam1']:.0e}, lam2={best_params['lam2']:.0e}, NRMS={best_error:.4e}"
        )
...
def _build_fir_fit_figure_auto(
    ...
    ax.axvline(info_fwd["L"], color="r", ls="--", lw=1.5, alpha=0.7, label=f"Chosen L={info_fwd['L']} (AIC)")
...
def analyze_and_plot_inverse_fir_auto(
    ...
    _, h_inv_final, best_reconstructed, _ghost_fig, fig_inverse, replay_params = analyze_and_plot_inverse_fir(
        response=response,
        time=time,
        Ts=Ts,
        L_values=[info_fwd["L"]],
