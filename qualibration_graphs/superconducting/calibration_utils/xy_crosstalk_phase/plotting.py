...
def plot_raw_data_with_fit(ds: xr.Dataset, qubits: List[AnyTransmon], fits: xr.Dataset):
    ...
    for role in ds["role"]:
        ...
        grid.fig.suptitle(f"XY Crosstalk Phase - {st_}")
...
def plot_individual_data(ax: Axes, ds: xr.Dataset, qubit: dict[str, str], fit: xr.Dataset = None):
    ...
    ds.sel(qubit)[data].plot(
        ax=ax, add_colorbar=False, x="pulse_duration", y="relative_phase", robust=True
    )
    ax.set_ylabel(f"Relative phase [2pi]")
