...
5. **Success only checks that `freq_offset` and `decay` are non-NaN** — unlike `05_T1`, there is no magnitude/plausibility bound (e.g. nothing rejects a fitted $T_2^*$ far larger than `max_wait_time_in_ns`). See [General Troubleshooting](_general_troubleshooting.md#general-troubleshooting) #1.
