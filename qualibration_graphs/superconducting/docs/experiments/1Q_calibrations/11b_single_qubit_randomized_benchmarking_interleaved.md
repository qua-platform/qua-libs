...
2. **Run-to-run scatter in the fitted fidelity for a fixed gate is large** → too few `num_random_sequences` (default 100, lower than `11a`'s 300 given the smaller default `max_circuit_depth`) — a shot-noise-limited symptom, not necessarily a real problem with that gate. Increase `num_random_sequences` before concluding the gate is unstable.
