...
| `amp_factor_step` | `float` | 0.02 | – (pre-factor) | Step size of the $\alpha$ pre-factor sweep. | Finer steps resolve the minimum more precisely, at proportional run-time cost; too coarse a step can under-resolve the sharp, error-amplified minimum at high `nb_of_pulses` (see Parameter Tuning Heuristics #5). |
