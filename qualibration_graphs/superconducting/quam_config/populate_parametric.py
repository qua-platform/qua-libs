...
for k, qubit in enumerate(machine.qubits.values()):
    ...
    if qubit.resonator.opx_output.band in (1, 3):
        qubit.resonator.opx_output.delay = 26
...
xy_freq = np.array([6.012, 6.421, 6.785, 7.001, 7.083]) * u.GHz
xy_LO = np.array([6.0, 6.1, 6.5, 6.8, 7.1]) * u.GHz
...
for k, qubit in enumerate(machine.qubits.values()):
    ...
    if qubit.xy.opx_output.band in (1, 3):
        qubit.xy.opx_output.delay = 26
...
for qp in qubit_pairs:
    machine.qubit_pairs[qp].coupler.intermediate_frequency = 100e6
    machine.qubit_pairs[qp].coupler.opx_output.upsampling_mode = "mw"
    machine.qubit_pairs[qp].coupler.opx_output.output_mode = "direct"
    machine.qubit_pairs[qp].coupler.opx_output.delay = 174
