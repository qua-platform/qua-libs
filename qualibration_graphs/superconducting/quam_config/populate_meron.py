...
rr_freq = np.array([7.81425, 7.67145, 7.5332, 7.233250000]) * u.GHz
rr_LO = 7.4 * u.GHz
...
xy_freq = np.array([6.245652507746367, 5.983947025905643, 5.657857623, 6.236056991]) * u.GHz
xy_LO = np.array([6.1, 6.1, 5.6, 6.1]) * u.GHz
...
anharmonicity = np.array([215, 215, 215, 215]) * u.MHz
...
for k, qubit in enumerate(machine.qubits.values()):
    ...
    qubit.xy.opx_output.full_scale_power_dbm = 10  # Max drive power in dBm
...
for k, qubit in enumerate(machine.qubits.values()):
    if hasattr(qubit, "z"):
        qubit.z.opx_output.output_mode = "direct"  # Flux line output mode
...
for k, q in enumerate(machine.qubits):
    ...
    if hasattr(machine.qubits[q], "z"):
        ...
        machine.qubits[q].z.opx_output.delay = 94
