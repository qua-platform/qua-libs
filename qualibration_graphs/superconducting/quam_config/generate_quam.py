...
q1to2_res_ch = mw_fem_spec(con=1, slot=1, in_port=1, out_port=1)
# q5to8_res_ch = mw_fem_spec(con=1, slot=2, in_port=1, out_port=1)
# individual xy drive for qubits 1 to 4 on MW-FEM 1
q1to2_drive_ch = mw_fem_spec(con=1, slot=1, in_port=None, out_port=None)
# multiplexed xy drive for qubits 5 to 8 on MW-FEM 2 port 4
# q5to8_drive_ch = mw_fem_spec(con=1, slot=2, in_port=None, out_port=4)
# individual xy drive for twpa 1 to 2 on MW-FEM 2 
twpa1to2_drive_ch = mw_fem_spec(con=1, slot=2, in_port=None, out_port=None)
########################################################################################################################
# %%                Allocate the wiring to the connectivity object based on the available instruments
########################################################################################################################
connectivity = Connectivity()
# The readout lines
connectivity.add_resonator_line(qubits=qubits[:4], constraints=q1to2_res_ch)
# connectivity.add_resonator_line(qubits=qubits[4:], constraints=q5to8_res_ch)
# The xy drive lines
connectivity.add_qubit_drive_lines(qubits=qubits[:4], constraints=q1to2_drive_ch)
...
# The TWPA drive lines
connectivity.add_twpa_drive_lines(twpas=twpas, constraints=twpa1to2_drive_ch)
