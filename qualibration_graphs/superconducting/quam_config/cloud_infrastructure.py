...
class CloudQuantumMachine:
    ...
    def execute(self, program):
        run_data = self._qc.execute(program, self._config, options={"timeout": 600}, terminal_output=True)
    ...
    def get_jobs(self, status: list[str]):
        if status == ["Running"]:
            return self.job.result_handles.is_processing()
        else:
            return False
