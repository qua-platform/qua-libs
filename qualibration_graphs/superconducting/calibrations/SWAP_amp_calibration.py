...
with program() as SWAP_amp_calibration:
    ...
    with stream_processing():
        ...
        for i in range(num_qubit_pairs):
            if node.parameters.use_state_discrimination:
                state_st_control[i].buffer(len(control_amps)).buffer(len(N_pi_vec)).average().save(
                    f"state_control{i + 1}")
                state_st_target[i].buffer(len(control_amps)).buffer(len(N_pi_vec)).average().save(
                    f"state_target{i + 1}")
...
if not node.parameters.simulate:
    ...
    if N_pi == 1:
        ...
        for qp in qubit_pairs:
            node.results["results"][qp.name] = {}
            node.results["results"][qp.name]["SWAP_amplitude"] = qp.gates[
                                                                     "SWAP_Coupler"].flux_pulse_control.amplitude * float(
                ds.amp[data_max_idx.sel(qubit=qp.name)].values)
