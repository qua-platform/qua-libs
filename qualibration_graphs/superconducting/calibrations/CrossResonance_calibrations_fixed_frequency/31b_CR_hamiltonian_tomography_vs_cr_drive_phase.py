...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    phases = np.arange(
        node.parameters.min_cr_drive_phase_2pi,
        node.parameters.max_cr_drive_phase_2pi,
        node.parameters.step_cr_drive_phase_2pi,
    )
    ...
    with program() as node.namespace["qua_program"]:
        ...
        with stream_processing():
            ...
            for i, qp in enumerate(qubit_pairs):
                ...
                ).buffer(len(phases)).average().save(f"state_c{i + 1}")
                state_t_st[i].buffer(len(control_state)).buffer(len(qst_basis)).buffer(
                    len(pulse_duration_clocks)
                ).buffer(len(phases)).average().save(f"state_t{i + 1}")
    ...
    with node.record_state_updates():
        for multiplexed_qubit_pairs in node.namespace["qubit_pairs"].batch():
            for i, qp in multiplexed_qubit_pairs.items():
                ...
                # # quam macro for cr
                # qp.macros["cr"].cr_drive_phase = 0.23
                pass
