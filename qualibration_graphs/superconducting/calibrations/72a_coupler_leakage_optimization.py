...
from qualibration_libs.legacy.lib.fit import fit_oscillation, fix_oscillation_phi_2pi, oscillation
...
from qualibration_libs.legacy.macros import (
    active_reset,
    active_reset_gef,
    active_reset_simple,
    readout_state,
...
class Parameters(NodeParameters):
    ...
    # wide scan:
    qubit_flux_min: float = -0.015
    qubit_flux_max: float = 0.015

    qubit_flux_step: float = 0.0005
    use_state_discrimination: bool = True


node = QualibrationNode(name="72a_coupler_leakage_optimization", parameters=Parameters())
assert not (
...
with program() as CPhase_Oscillations:
    ...
    for i, qp in enumerate(qubit_pairs):
        ...
        update_frequency(
            qp.qubit_control.resonator.name,
            qp.qubit_control.resonator.intermediate_frequency + qp.qubit_control.resonator.GEF_frequency_shift,
        )
        wait(4)
...
if not node.parameters.simulate:
    ...
    for ax, qp in grid_iter(grid):
        ...
        base_detuning = (
            -machine.qubit_pairs[qp["qubit"]].detuning ** 2
            * machine.qubit_pairs[qp["qubit"]].qubit_control.freq_vs_flux_01_quad_term
        )
