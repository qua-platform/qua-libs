...
node = QualibrationNode[Parameters, Quam](
    name="10c_stark_calibration_180_minus_180",
...
def custom_param(node: QualibrationNode[Parameters, Quam]):
    ...
    node.parameters.qubits = ["qA3"]
    node.parameters.alpha_setpoint = -1.0
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    with program() as node.namespace["qua_program"]:
        ...
        a2 = declare(fixed)  # QUA variable for the qubit drive amplitude pre-factor
        npi = declare(int)  # QUA variable for the number of qubit pulses
        df = declare(int)  # QUA variable for the number of qubit pulses
                ...
                with for_(*from_array(npi, N_pi_vec)):
                    with for_(*from_array(df, dfs)):
                        with for_(*from_array(a, amps)):
                            ...
                            # Qubit manipulation
                            for i, qubit in multiplexed_qubits.items():
                                assign(a2, a * Math.div(int(qubit.anharmonicity), (int(qubit.anharmonicity) + df)))
                                ...
                                with for_(count, 0, count < npi, count + 1):
                                    if node.parameters.operation == "x180":
                                        play(
                                            node.parameters.operation * amp(1, 0, 0, a2),
                                            qubit.xy.name,
                                        )
                                        play(
                                            node.parameters.operation * amp(-1, 0, 0, -a2),
        ...
        with stream_processing():
            ...
            for i, qubit in enumerate(qubits):
                if node.parameters.use_state_discrimination:
                    state_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(len(N_pi_vec)).average().save(f"state{i + 1}")
                else:
                    I_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(N_pi).average().save(f"I{i + 1}")
                    Q_st[i].buffer(len(amps)).buffer(len(dfs)).buffer(N_pi).average().save(f"Q{i + 1}")
...
def analyse_data(node: QualibrationNode[Parameters, Quam]):
    ...
    plt.figure()
    node.results["ds_raw"].sel(qubit="qA3").sum("nb_of_pulses").state.plot()
    plt.show()
