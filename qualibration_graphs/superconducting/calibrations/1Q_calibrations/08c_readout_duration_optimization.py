...
from calibration_utils.readout_duration_optimization import (
    Parameters as Parameters, process_raw_dataset, fit_raw_data, log_fitted_results, plot_results,
)
from calibration_utils.readout_duration_optimization import experiment

# %% {Description}
description = """READOUT DURATION OPTIMIZATION
Sweep square readout pulse duration at fixed amplitude/frequency, acquiring individual g/e IQ shots.
As in 08b, maximize GMM assignment fidelity subject to the non-outlier fraction criterion.
At the selected duration, recalibrate IQ rotation, GE/RUS thresholds and confusion matrix.
Prerequisites: calibrated readout frequency/power and x180; default square integration weights.
Updates: readout.length, integration_weights_angle, threshold, rus_exit_threshold,
and resonator.confusion_matrix. Equal fidelities favor the shortest duration.
...
node = QualibrationNode[Parameters, Quam](
    name="08c_readout_duration_optimization", description=description, parameters=Parameters(), machine=Quam.load(),
)


@node.run_action(skip_if=node.modes.external)
def custom_param(node: QualibrationNode[Parameters, Quam]):
    node.parameters.qubits = ["qC4", "qA2"]
