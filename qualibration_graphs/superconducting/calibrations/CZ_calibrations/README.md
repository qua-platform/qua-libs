...
| 2     | **33a** | Fine amplitude scan → π conditional-phase point                                               |
| 3     | **33b** | CZ pulse train → error-amplified amplitude fine tune                                          |
| 4     | **34a** | Virtual-Z phase compensation (can run after **33a**; **99** runs it in parallel with **33b**) |
...
Node **30** finds coupler **decouple (idle)** and **interaction** flux biases plus moving-qubit detuning in one 2D map (CZ or iSWAP). That replaces the coarse amplitude/duration role of chevron (**31**), so the usual path is **30 → 32a → 32b → 33a → 33b → 34a** without **31**. Pulse duration and macro amplitudes come from **30** and the gate macro already in QUAM. Use **32b** (PALEA) instead of **32a** for improved leakage isolation.
...
| 4     | **33a** | Fine amplitude → π conditional-phase point                                          |
| 5     | **33b** | CZ pulse train → error-amplified amplitude fine tune                                |
| 6     | **34a** | Virtual-Z phase compensation                                                        |
| 6′    | **34b** | Error-amplified virtual-Z fine tune (optional follow-up to **34a**)                 |
...
Use gate duration from **31** (fixed coupler) or from the macro in state after **30** (tunable coupler). On tunable couplers, run after leakage calibration (**32a** or **32b**). Sweep amplitude to the **π conditional-phase** point (phase difference = 0.5 in normalised units). Frame tomography uses rotating x90 on the stationary qubit.
...
| **99**  | [`99_CZ_calibration_graph.py`](./99_CZ_calibration_graph.py)                         | ✓ (31–33b–34a) | — (use **30** → 32a–32b–33a/b–34a by hand or custom graph) |
...
- **31** → **33a** → **33b** → **34a** (add **34b** manually for finer virtual-Z tuning)
