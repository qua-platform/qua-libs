...
# %% {Initialisation}
description = """
This experiment performs full quantum state tomography of a two-qubit Bell state to characterize
the fidelity and properties of the entangled state generated by an entangling gate (e.g. CZ).

The sequence prepares a Bell state, applies a complete set of tomography rotations on both qubits,
and measures joint outcomes in the computational basis to reconstruct the two-qubit density matrix.

Process:
1. Prepare both qubits in the ground state
2. Create the Bell state using single-qubit rotations and an entangling gate (e.g. CZ)
3. Apply tomography rotations (I, X, Y, Z combinations) on both qubits
4. Measure both qubits and reconstruct the density matrix from expectation values

Metrics:
- State fidelity with respect to the target Bell state
- Purity and population/coherence terms of the density matrix

Readout mitigation:
- Kron: tensor product of per-qubit resonator confusion matrices
- Joint: two-qubit confusion matrix from node 35_two_qubit_confusion_matrix
...
State update:
- Bell-state fidelity and purity saved to macros[operation].fidelity["Bell_State"] for each
  successfully analysed pair
"""
...
def custom_param(node: QualibrationNode[Parameters, Quam]):
    ...
    node.parameters.qubit_pairs = ["qC2-qC1"]
    pass
