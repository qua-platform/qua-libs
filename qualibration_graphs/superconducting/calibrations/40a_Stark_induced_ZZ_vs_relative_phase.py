...
from calibration_utils.stark_zz_vs_relative_phase import (
    Parameters,
    process_raw_dataset,
    fit_raw_data,
    log_fitted_results,
...
node = QualibrationNode[Parameters, Quam](
    name="40a_Stark_induced_ZZ_vs_relative_phase",  # Name should be unique
...
def custom_param(node: QualibrationNode[Parameters, Quam]):
    ...
    node.parameters.min_zz_drive_relative_phase_2pi = -1.0
    node.parameters.max_zz_drive_relative_phase_2pi = +1.0
...
def create_qua_program(node: QualibrationNode[Parameters, Quam]):
    ...
    cosines = np.cos(2 * np.pi * relative_phases)
    ...
    node.namespace["sweep_axes"] = {
        ...
        "relative_phases": xr.DataArray(relative_phases, attrs={"long_name": "stark zz drive relative phase"}),
    ...
    with program() as node.namespace["qua_program"]:
        ...
        i = declare(int)
        cos = declare(float, value=cosines.tolist()) # QUA variable for amplitude scaling
        sin = declare(float, value=sines.tolist()) # QUA variable for amplitude scaling
                    ...
                    with for_(*from_array(t, idle_times)):
                        ...
                        with for_(s, 0, s < 2, s + 1): # states 0:g or 1:e
                            ...
                            for i, qp in multiplexed_qubit_pairs.items():
                                ...
                                qt.xy_detuned.play(
                                    f"zz_flattop_{qp.name}",
                                    amplitude_scale=[cos[i], -sin[i], sin[i], cos[i]],
                                ) # sweep amplitude
                                # IZ, ZI correction
