    ...
    with program() as node.namespace["qua_program"]:
        ...
        with stream_processing():
            ...
            for i in range(num_qubits):
                if state_discrimination:
                    state_d_st[i].buffer(len(pulse_durations)).buffer(len(amp_scalings)).average().save(
                        f"state_d{i + 1}"
                    )
                    state_p_st[i].buffer(len(pulse_durations)).buffer(len(amp_scalings)).average().save(
                        f"state_p{i + 1}"
                    )
                else:
