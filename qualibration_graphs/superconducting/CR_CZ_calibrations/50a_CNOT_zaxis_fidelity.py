...
from calibration_utils.cnot_zaxis import (
    Parameters,
    process_raw_dataset,
    fit_raw_data,
    log_fitted_results,
...
# %% {Description}
description = """
    CNOT Z-AXIS FIDELITY
Measure the fidelity of the CNOT gate by estimating the 4×4 confusion matrix of input vs. output states.  
Each of the four computational basis states (|00⟩, |01⟩, |10⟩, |11⟩) is prepared, followed by the CNOT sequence  
(Hadamard on control, Stark-CZ, Hadamard on control). The resulting states are measured in the Z basis.  
The gate fidelity is extracted as 1 minus the total probability weight of non-ideal outcomes.
Namely, estimated CNOT fidelity is equal to (1 − error probability).

Prerequisites:
    - Calibrated Stark-CZ gate parameters (τ_p, ω_d, amplitudes, φ_d).
    - Calibrated single-qubit rotations (X, Y gates) and state discrimination.
    - Reset and readout tuned for high-fidelity measurement.
"""
