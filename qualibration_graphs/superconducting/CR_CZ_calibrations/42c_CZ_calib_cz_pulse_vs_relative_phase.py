...
from calibration_utils.stark_cz_vs_relative_phase import (
    Parameters,
    process_raw_dataset,
    fit_raw_data,
    log_fitted_results,
...
# %% {Description}
description = """
    CZ CALIB CZ PULSE VS RELATIVE PHASE
Fine-tune the relative drive phase (φ_d) of the Stark-CZ gate with τ_p, ω_d, and amplitudes fixed.  
Prepare Qc in |0⟩/|1⟩, place Qt on the equator (Y/2), apply Stark-CZ with varied φ_d, undo with −Y/2, and measure.

Prerequisites:
    - Fixed CZ duration, drive frequency, and amplitudes from prior calibrations.
    - Calibrated single-qubit gates, readout, and a stable phase reference between drive lines.

State Update:
    - Select optimal relative phase φ_d minimizing error.
    - Update Stark-CZ waveform parameter with the refined relative phase.
      (qt.xy_detuned.operations[f"zz_{wf_type}_{qp.name}"].axis_angle)
"""


# Be sure to include [Parameters, Quam] so the node has proper type hinting
node = QualibrationNode[Parameters, Quam](
    name="42b_CZ_calib_cz_pulse_vs_relative_phase",  # Name should be unique
    ...
    with program() as node.namespace["qua_program"]:
        ...
        with stream_processing():
            ...
            for i, qp in enumerate(qubit_pairs):
                state_c_st[i].buffer(2).buffer(len(relative_phases)).average().save(f"state_c{i + 1}")
                state_t_st[i].buffer(2).buffer(len(relative_phases)).average().save(f"state_t{i + 1}")
