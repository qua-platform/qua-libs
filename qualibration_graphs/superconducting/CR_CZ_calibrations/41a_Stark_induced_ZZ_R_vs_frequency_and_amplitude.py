...
from calibration_utils.stark_zz_R_vs_freq_and_amp import (
    Parameters,
    process_raw_dataset,
    fit_raw_data,
    log_fitted_results,
...
# %% {Description}
description = """
    STARK INDUCED ZZ R VS FREQUENCY AND AMPLITUDE
Calibrate CZ strength using the metric R = ½‖r0 − r1‖ from Qt tomography while sweeping Stark-CZ drive frequency (ω_d) and amplitude (A).  
For each (ω_d, A), prepare Qc in |0⟩/|1⟩, put Qt on the equator, apply Stark-CZ (fixed τ_p, φ_d), do Qt tomography, and compute R.

Prerequisites:
    - Prior ZZ scans to set τ_p and φ_d.
    - Calibrated single-qubit tomography, readout, and mixer/Octave settings.
    - Known safe ω_d band.

State Update:
    - Choose (ω_d*, A*) from the R ≈ 1 region (maximal entangling strength).
    - Update:
        zz_control.detuning
        zz_target.detuning
        if calibrate_qc_amp_scaling: zz_control.operations[wf_type].amplitude
...
# Any parameters that should change for debugging purposes only should go in hered
    ...
    with program() as node.namespace["qua_program"]:
                        ...
                        with for_(b_t, 0, b_t < 3, b_t + 1):  # tomographic basis for qt
                            with for_(s, 0, s < 2, s + 1):  # states 0:g or 1:e
                                for i, qp in multiplexed_qubit_pairs.items():
                                    ...
                                    # Reset the qubits to the ground state
                                    qc.reset(
                                        node.parameters.reset_type, node.parameters.simulate, log_callable=node.log
                                    )
                                    qt.reset(
                                        node.parameters.reset_type, node.parameters.simulate, log_callable=node.log
                                    )
        ...
        with stream_processing():
            ...
            for i, qp in enumerate(qubit_pairs):
                state_c_st[i].buffer(2).buffer(3).buffer(len(amp_scalings)).buffer(len(dfs)).average().save(
                    f"state_c{i + 1}"
                )
                state_t_st[i].buffer(2).buffer(3).buffer(len(amp_scalings)).buffer(len(dfs)).average().save(
                    f"state_t{i + 1}"
                )
