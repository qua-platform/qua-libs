...
for arg in ['Pe_0', 'Pe_corrected']:
    ...
    with program() as prog:
        ...
        with stream_processing():
            freq_track_obj.state_estimation_st[0].buffer(reps, len(freq_track_obj.tau_vec)).map(FUNCTIONS.average()).save(arg)
...
with program() as prog:
    freq_track_obj.qua_declarations()
    freq_track_obj.freq_domain_ramsey_full_sweep(reps, freq_track_obj.fres - 2*freq_track_obj.f_ref,
                                                 freq_track_obj.fres + 2*freq_track_obj.f_ref,
                                                 int(0.002e6), 'Pe_fd', oscillation)
    with stream_processing():
        freq_track_obj.state_estimation_st[0].buffer(reps, len(freq_track_obj.fvec)).map(FUNCTIONS.average()).save('Pe_fd')
...
with program() as prog:
    ...
    with stream_processing():
        freq_track_obj.state_estimation_st[0].buffer(reps, len(freq_track_obj.tau_vec)).map(FUNCTIONS.average()).save('Pe_td_ref')
        freq_track_obj.state_estimation_st[1].buffer(reps, len(freq_track_obj.tau_vec)).map(FUNCTIONS.average()).save('Pe_td_corr')
