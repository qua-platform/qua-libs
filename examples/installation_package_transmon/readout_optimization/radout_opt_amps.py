...
with program() as rr_opt_a:
    ...
    with stream_processing():
        ...
        (((Ig_st.buffer(len(amps))*Ig_st.buffer(len(amps))).average()) -
         (Ig_st.buffer(len(amps)).average()*Ig_st.buffer(len(amps)).average())).save('Igvar')
        (((Qg_st.buffer(len(amps))*Qg_st.buffer(len(amps))).average()) -
         (Qg_st.buffer(len(amps)).average()*Qg_st.buffer(len(amps)).average())).save('Qgvar')
        (((Ie_st.buffer(len(amps))*Ie_st.buffer(len(amps))).average()) -
         (Ie_st.buffer(len(amps)).average()*Ie_st.buffer(len(amps)).average())).save('Ievar')
        (((Qe_st.buffer(len(amps))*Qe_st.buffer(len(amps))).average()) -
         (Qe_st.buffer(len(amps)).average()*Qe_st.buffer(len(amps)).average())).save('Qevar')
