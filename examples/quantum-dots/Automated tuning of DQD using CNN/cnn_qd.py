from configuration import config

from qm.qua import *
from qm.QuantumMachinesManager import QuantumMachinesManager, SimulationConfig

qmm = QuantumMachinesManager()

v1_start = -0.2
v1_end = 0
v2_start = -0.15
v2_end = 0.10
step = 0.001
n_v1 = int((v1_end - v1_start) / step)
n_v2 = int((v2_end - v2_start) / step)
N = 1000


def scan2d(v1_start, v1_end, v2_start, v2_end, step, N):
    with program() as prog:
        v1 = declare(fixed)
        v2 = declare(fixed)
        I = declare(fixed)
        n = declare(int)
        I_avg = declare_stream()
        with for_(n, 0, n < N, n + 1):
            with for_(v1, v1_start, v1 < v1_end, v1 + step):
                with for_(v2, v2_start, v2 < v2_end, v2 + step):
                    align("PG1", "PG2", "QPC")
                    play("playOp", "PG1")
                    play("playOp", "PG2")
                    measure("readout", "QPC", None, integration.full("integW", I, "out1"))
                    save(I, I_avg)
        with stream_processing():
            I_avg.buffer(n_v1, n_v2).average().save("result")
    return prog


job = qmm.simulate(config, scan2d(v1_start, v1_end, v2_start, v2_end, step, N), SimulationConfig(int(1e4)))
result = job.result_handles.result.fetch_all()['value']
