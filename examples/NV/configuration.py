...
pi_wf = (pi_amp * (signal.windows.gaussian(pi_len, pi_len / 5) -
                   signal.windows.gaussian(pi_len, pi_len / 5)[-1])).tolist()  # waveform

# Pi_half pulse parameters
pi_half_len = 60  # in units of ns
pi_half_amp = 0.15  # in units of volts
pi_half_wf = (pi_half_amp * (signal.windows.gaussian(pi_half_len, pi_half_len / 5) -
                             signal.windows.gaussian(pi_half_len, pi_half_len / 5)[-1])).tolist()  # waveform
...
gauss_wf = (gauss_amp * (signal.windows.gaussian(gauss_len, gauss_len / 5) -
                         signal.windows.gaussian(gauss_len, gauss_len / 5)[-1])).tolist()  # waveform

# Pi_aux pulse parameters
pi_aux_len = 60  # in units of volts
pi_aux_amp = 0.3  # in units of ns
pi_aux_wf = (pi_amp * (signal.windows.gaussian(pi_len, pi_len / 5) -
                   signal.windows.gaussian(pi_len, pi_len / 5)[-1])).tolist()  # waveform
...
config = {
    ...
    'mixers': {
        'mixer_qubit': [
            {'intermediate_frequency': qubit_freq, 'lo_frequency': LO_freq, 'correction': IQ_imbalance(0, 0)},
        ],
        'mixer_qubit_aux': [
            {'intermediate_frequency': aux_freq, 'lo_frequency': LO_aux_freq, 'correction': IQ_imbalance(0, 0)},
        ],
    },
}
