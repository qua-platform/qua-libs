...
def calc_corr(corr_in1, corr_in2, in_len, fold):
    ...
    with for_(i_iter1, 1, i_iter1 < in_len, i_iter1+1):
        with for_(i_iter2, 0, i_iter2 < i_iter1, i_iter2+1):
            ...
            assign(corr_out[out_ind], corr_out[out_ind] + (corr_in2[i_iter2]*corr_in1[in_len-i_iter1+i_iter2]))
    ...
    with for_(i_iter1, 1, i_iter1 < in_len, i_iter1+1):
        with for_(i_iter2, 0, i_iter2 < (in_len-i_iter1), i_iter2+1):
            ...
            assign(corr_out[out_ind], corr_out[out_ind] +
                   (corr_in2[i_iter2+i_iter1]*corr_in1[i_iter2]))
...
with program() as prog1:
    ...
    # program
    measure("measurement", "qe1", 'stream1', time_tagging.raw(time_tag_vec1, window_size, tag_num1))
    measure("measurement", "qe2", 'stream2', time_tagging.raw(time_tag_vec2, window_size, tag_num2))
...
if fold:
    ...
    corr_output_validate_temp[1:bin_vec_len] = np.flip(corr_output_validate[0:bin_vec_len-1]) + \
