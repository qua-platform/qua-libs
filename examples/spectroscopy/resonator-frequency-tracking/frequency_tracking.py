...
def track_frequency(qe, pulse, current_w, w_step):
    ...
    assign(steps_position, Util.cond(res_w_smaller < res_w, steps_position - 1,
...
with program() as w_if_tracking:
    ...
    with for_(loop_iteration_counter, 0, loop_iteration_counter < 100, loop_iteration_counter + 1):
...
plt.plot([time for (magnitude, time) in response_values], [magnitude for (magnitude, time) in response_values], '--bo')
plt.show()
