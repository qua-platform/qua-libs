...
displace_I = gauss(np.real(alpha), displace_len / 2 - 3 * int(sigma_displace + 1), sigma_displace, displace_len)
displace_Q = gauss(np.imag(alpha), displace_len / 2 - 3 * int(sigma_displace + 1), sigma_displace, displace_len)
...
config = {
    ...
    'waveforms': {
        ...
        'Idis_wf': {
            'type': 'arbitrary',
            'samples': [float(arg) for arg in displace_I]
        },

        'Qdis_wf': {
            'type': 'arbitrary',
            'samples': [float(arg) for arg in displace_Q]
        },
    },
    ...
    'mixers': {
        'mixer_cavity': [
            {'intermediate_frequency': cavity_IF, 'lo_frequency': lo_freq_cavity, 'correction': [1, 0, 0, 1]},
        ],

        'mixer_rr': [
            {'intermediate_frequency': rr_IF, 'lo_frequency': lo_freq_rr, 'correction': [1, 0, 0, 1]},
        ],

        'mixer_qubit': [
            {'intermediate_frequency': qubit_IF, 'lo_frequency': lo_freq_qubit, 'correction': [1, 0, 0, 1]},
        ],
    }
}
