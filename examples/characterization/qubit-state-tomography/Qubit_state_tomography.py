...
def do_tomography():
    with program() as tomography:
        stream_Ix = declare_stream()  # Open streams to allow data retrieval for post-processing
...
counts_1 = [np.count_nonzero(X), np.count_nonzero(Y),
            np.count_nonzero(Z)]  # Count the number of measurements of the |1> state for each axis
counts_0 = [total_counts[0] - counts_1[0], total_counts[1] - counts_1[1], total_counts[2] - counts_1[2]]  # Same for |0>
...
target = lambda x, y, z: L(x, y, z, counts_1[0], counts_0[0], counts_1[1], counts_0[1], counts_1[2], counts_0[2])
