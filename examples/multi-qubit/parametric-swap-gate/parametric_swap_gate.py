...
config = {
    ...
    'mixers': {
        'mixer_transmon_ge': [
            {'intermediate_frequency': transmon_IF, 'lo_frequency': LO_transmon,
             'correction': [1, 0, 0, 1]}
        ],
        'mixer_cubic_ge': [
            {'intermediate_frequency': cubic_IF, 'lo_frequency': LO_cubic,
             'correction': [1, 0, 0, 1]}
        ],
        'mixer_hybrid': [
            {'intermediate_frequency': ge_eg_IF, 'lo_frequency': LO_hybrid,
             'correction': [1, 0, 0, 1]},
            {'intermediate_frequency': ee_gf_IF, 'lo_frequency': LO_hybrid,
             'correction': [1, 0, 0, 1]}
        ],
        'mixer_rr': [
            {'intermediate_frequency': rr_transmon_IF, 'lo_frequency': rr_LO,
             'correction': [1, 0, 0, 1]},
            {'intermediate_frequency': rr_cubic_IF, 'lo_frequency': rr_LO,
             'correction': [1, 0, 0, 1]},
        ],
    }
...
with program() as swap:
    ...
    with for_(n, 0, n < 10000, n+1):
        ...
        with for_(phi, -np.pi, phi < np.pi, phi + np.pi/50):
            ...
            measure("readout", "rr_cubic", None, demod.full('integW_cos', I),
