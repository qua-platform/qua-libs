...
class NNStateDiscriminator:
    ...
    def initialize_qua(self):
        """
        Initializes Qua variables to be used by the measure_state function
        """
        qua_vars = dict()
        qua_vars["out1"] = declare(fixed, size=self.rr_num)
        qua_vars["out2"] = declare(fixed, size=self.rr_num)
        qua_vars["res"] = declare(fixed, size=self.num_of_states)
    ...
    def measure_state(self, readout_op, result, qua_vars, adc=None):
        ...
        :param qua_vars: a dictionary generated by the initialize_qua() routine, executed outside the measure statement
