...
class TimeDiffCalibrator:
    ...
    def _update_config(freq, config, qe):
        ...
        print(f"ATTENTION: Using the mixer at the 0'th position of {config['elements'][qe]['mixInputs']['mixer']} to "
        ...
        config["digital_waveforms"]['time_diff_ON'] = {
            'samples': [(1, 0)]
        }
        config["waveforms"]["time_diff_const_wf"] = {
            'type': 'constant',
            'sample': 0.4
        }
        config["waveforms"]["time_diff_zero_wf"] = {
            'type': 'constant',
            'sample': 0.0
        }
