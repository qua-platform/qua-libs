...
with program() as time_rabi:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        # Neutral state readout (bright)
        assign(counts_bright, 0)
        play('charge_init', 'laser_705nm')  # charge initialization (bright state)
        align('laser_705nm', 'laser_EX12', 'SNSPD')
        play('charge_read', 'laser_EX12')  # Charge readout
        measure('photon_count', 'SNSPD', None,
                time_tagging.analog(times_bright, meas_len, counts_bright))  # photon count
        save(counts_bright, counts_bright_st)  # save counts

        # Charged state readout (neutral)
        align()  # global align
        assign(counts_dark, 0)
        play('ionization', 'laser_EX121151nm')  # agnostic ionization (dark state)
        align('laser_EX121151nm', 'laser_EX12', 'SNSPD')
        play('charge_read', 'laser_EX12')  # Charge readout
        measure('photon_count', 'SNSPD', None,
                time_tagging.analog(times_dark, meas_len, counts_dark))  # photon count
