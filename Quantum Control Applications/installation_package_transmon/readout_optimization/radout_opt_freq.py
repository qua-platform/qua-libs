...
with program() as rr_opt_f:
    ...
    with stream_processing():
        ...
        (((Ig_st.buffer(len(freqs))*Ig_st.buffer(len(freqs))).average()) -
         (Ig_st.buffer(len(freqs)).average()*Ig_st.buffer(len(freqs)).average())).save('Igvar')
        (((Qg_st.buffer(len(freqs))*Qg_st.buffer(len(freqs))).average()) -
         (Qg_st.buffer(len(freqs)).average()*Qg_st.buffer(len(freqs)).average())).save('Qgvar')
        (((Ie_st.buffer(len(freqs))*Ie_st.buffer(len(freqs))).average()) -
         (Ie_st.buffer(len(freqs)).average()*Ie_st.buffer(len(freqs)).average())).save('Ievar')
        (((Qe_st.buffer(len(freqs))*Qe_st.buffer(len(freqs))).average()) -
         (Qe_st.buffer(len(freqs)).average()*Qe_st.buffer(len(freqs)).average())).save('Qevar')
