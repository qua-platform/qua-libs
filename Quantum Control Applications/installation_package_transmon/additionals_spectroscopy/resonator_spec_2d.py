...
with program() as resonator_spec_2d:
    ...
    a = declare(fixed)  # variable to change the amplitude of the signal
...
if simulate:
    ...
    simulate_config = SimulationConfig(duration=1000
                                       , simulation_interface=LoopbackInterface(([('con1', 1, 'con1', 1)])))
    job = qmm.simulate(config, resonator_spec_2d, simulate_config)  # do simulation with qmm
    job.get_simulated_samples().con1.plot()  # visualize played pulses

else:
    job = qm.execute(resonator_spec_2d)  # execute QUA program
    ...
    while res_handles.is_processing():
        try:
            ...
            Z = I + Q*1j
            plt.title('2D resonator spectroscopy analysis')
            plt.pcolor(freqs, amps, np.sqrt(np.abs(Z)))
            plt.xlabel('IF [Hz]')
            plt.ylabel('Amps')
            plt.pause(0.1)
            plt.clf()
