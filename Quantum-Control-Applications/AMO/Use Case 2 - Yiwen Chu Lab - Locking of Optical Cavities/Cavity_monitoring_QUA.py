...
def PID_monitor_prog(
    ...
    with program() as prog:
        ...
        with for_(n_outer_repeat, 0, n_outer_repeat < N_outer_repeat, n_outer_repeat + 1):
            ...
            with for_each_(
                pid_on, [True, False]
            ):  # just changed the order of the True and False for the locking to happen before the unlocking
                ...
                with for_(n_inner_repeat, 0, n_inner_repeat < N_inner_repeat, n_inner_repeat + 1):
                    ...
                    assign(
                        dc_offset_1, dc_offset_1 + correction
                    )  # The way "offset" works is that it adds up correction to the previous value, because it's a "sticky". See "sticky" QUA documentation

                    # Apply the correction
                    play(
                        "offset" * amp(correction * 4), "filter_cavity_1"
                    )  # Note that the previously sent analog signal at the end of the pulse is played until the next pulse is sent, done through using a QUA sticky element. Search "sticky" in the configuration_cavity_locking_ETHZ_OPX1.py file.
        ...
        with stream_processing():
            ...
            integrator_error_st.buffer(
                int(lock_time * 1e9 / readout_len) + int(wait_time * 1e9 / readout_len)
            ).save_all("integration_error")
            derivative_error_st.buffer(
                int(lock_time * 1e9 / readout_len) + int(wait_time * 1e9 / readout_len)
            ).save_all("derivative_error")
            single_shot_st.buffer(int(lock_time * 1e9 / readout_len) + int(wait_time * 1e9 / readout_len)).save_all(
                "single_shot"
            )
            offset_st.buffer(int(lock_time * 1e9 / readout_len) + int(wait_time * 1e9 / readout_len)).save_all("offset")
            single_shot_AC_st.buffer(int(lock_time * 1e9 / readout_len) + int(wait_time * 1e9 / readout_len)).save_all(
                "single_shot_AC"
            )
            ...
            target_st.buffer(int(lock_time * 1e9 / readout_len) + int(wait_time * 1e9 / readout_len)).save_all(
                "target_fetch"
            )

            gain_P_st.timestamps().buffer(
                int(lock_time * 1e9 / readout_len) + int(wait_time * 1e9 / readout_len)
            ).save_all("timestamps")
...
if __name__ == "__main__":
    ...
    gain_D = 0.0
    N_repeat = 10
    prog = PID_monitor_prog(
        bitshift_scale_factor=set_bitshift,
        gain_P=gain_P,
        gain_I=gain_I,
        gain_D=gain_D,
        alpha=set_alpha,
        target=set_target,
        N_outer_repeat=N_repeat,
        lock_time=lockTime,
        wait_time=waitTime,
    )
    ...
    while results.is_processing():
        ...
        single_shot_DC_volts = -u.demod2volts(
            single_shot_DC, readout_len
        )  # invert DC because the OPX ADCs are inverted.
        single_shot_AC_volts = u.demod2volts(single_shot_AC, readout_len) * np.sqrt(
            2
    ...
    np.savez(
        f"QuantumMachine_UserCase/Cavity Locking Data/PID_monitoring_QUA {date}Gain_P{np.round(1e2*gain_P,decimals=2)}e-2_Gain_I{np.round(gain_I*1e4,decimals=2)}e-4_CustomAmp",
        error=error_data,
        integration_error=integration_error_data,
        derivative_error=derivative_error_data,
        single_shot_DC=single_shot_DC_V_data,
        offset=offset_data,
        single_shot_AC=single_shot_AC_data,
        gain_P=gain_P_data,
        gain_I=gain_I_data,
        timestamps=timestamps_data,
        params_dict=params_dict_data,
    )
