...
def main(N_repeat,gain_P,gain_I,gain_D,locktime,waittime,alpha,target,bitshift_scale_factor,lock_firsttime=0.5,N_repeat_firsttime=20,N_monitor_repeat=20):
    with program() as prog:
        ...
        with for_(n_repeat, 0, n_repeat < N_repeat, n_repeat + 1):
            ...
            wait_monitor("filters_transmission",single_shot_DC_transmission_st,single_shot_AC_transmission_st,I,Q,
                    waittime)
        ...
        with stream_processing():
            ...
            single_shot_DC_transmission_st.buffer(int(waittime*1e9 / readout_len)).save_all("single_shot_DC_transmission")
            single_shot_AC_transmission_st.buffer(int(waittime*1e9 / readout_len)).save_all("single_shot_AC_transmission")
            single_shot_DC_transmission_st.timestamps().buffer(int(waittime*1e9 / readout_len)).save_all("timestamps_transmission")
...
if __name__ == '__main__':
    ...
    if simulate_y_n:
        ...
        job = qm.execute(prog)
        results = fetching_tool(job, ["single_shot_DC_cav1","single_shot_AC_cav1","timestamps_cav1","single_shot_DC_cav2","single_shot_AC_cav2","timestamps_cav2","single_shot_DC_cav3","single_shot_AC_cav3","timestamps_cav3","single_shot_DC_transmission","single_shot_AC_transmission","timestamps_transmission"], mode="live")

        while results.is_processing():
            #Process data
            single_shot_DC_cav1,single_shot_AC_cav1,timestamps_cav1_fetched,single_shot_DC_cav2,single_shot_AC_cav2,timestamps_cav2_fetched,single_shot_DC_cav3,single_shot_AC_cav3,timestamps_cav3_fetched,single_shot_DC_transmission,single_shot_AC_transmission,timestamps_transmission_fetched = results.fetch_all()

            # Cavities reflection data
            single_shot_DC_cav1_volts = -u.demod2volts(single_shot_DC_cav1, readout_len) # invert DC because the OPX ADCs are inverted.
            single_shot_AC_cav1_volts = u.demod2volts(single_shot_AC_cav1, readout_len)*np.sqrt(2) #convert to voltage, still inverted
            ...
            single_shot_DC_cav2_volts = -u.demod2volts(single_shot_DC_cav2, readout_len) # invert DC because the OPX ADCs are inverted.
            single_shot_AC_cav2_volts = u.demod2volts(single_shot_AC_cav2, readout_len)*np.sqrt(2) #convert to voltage, still inverted
            ...
            single_shot_DC_cav3_volts = -u.demod2volts(single_shot_DC_cav3, readout_len) # invert DC because the OPX ADCs are inverted.
            single_shot_AC_cav3_volts = u.demod2volts(single_shot_AC_cav3, readout_len)*np.sqrt(2) #convert to voltage, still inverted
            ...
            single_shot_DC_transmission_volts = -u.demod2volts(single_shot_DC_transmission, readout_len) # invert DC because the OPX ADCs are inverted.
            single_shot_AC_transmission_volts = u.demod2volts(single_shot_AC_transmission, readout_len)*np.sqrt(2) #convert to voltage, still inverted
        ...
        print('Saving data...')
        np.savez(f"Cavity Locking Data/FullLock_data/3Cavities/3cavities_transmission_monitoring {date}Gain_P{np.round(1e2*gain_P,decimals=2)}e-2_Gain_I{np.round(gain_I*1e4,decimals=2)}e-4_CustomAmp",single_shot_DC_cav1=single_shot_DC_cav1_data,single_shot_AC_cav1=single_shot_AC_cav1_data,timestamps_cav1=timestamps_cav1_data,single_shot_DC_cav2=single_shot_DC_cav2_data,single_shot_AC_cav2=single_shot_AC_cav2_data,timestamps_cav2=timestamps_cav2_data,single_shot_DC_cav3=single_shot_DC_cav3_data,single_shot_AC_cav3=single_shot_AC_cav3_data,timestamps_cav3=timestamps_cav3_data,single_shot_DC_transmission=single_shot_DC_transmission_data,single_shot_AC_transmission=single_shot_AC_transmission_data,timestamps_transmission=timestamps_transmission_data)    
