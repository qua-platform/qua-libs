...
with program() as pulsed_odmr:
    ...
    counts_dark = declare(int)  # variable for number of counts
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(f, f_vec)):
            ...
            align()
            # Repeat with zero amplitude for the dark counts
            play("x180" * amp(0), "NV")
            # Align for laser readout after the MW pulse
            align()
            play("laser_ON", "AOM1")
            ...
            save(counts_dark, counts_dark_st)  # save counts on stream
            wait(wait_between_runs * u.ns)
...
if simulate:
    ...
    while results.is_processing():
        ...
        # Plot data
        plt.cla()
        plt.plot((NV_LO_freq * 0 + f_vec) / u.MHz, counts / 1000 / (meas_len_1 * 1e-9), label="photon counts")
        plt.plot((NV_LO_freq * 0 + f_vec) / u.MHz, counts_dark / 1000 / (meas_len_1 * 1e-9), label="dark counts")
