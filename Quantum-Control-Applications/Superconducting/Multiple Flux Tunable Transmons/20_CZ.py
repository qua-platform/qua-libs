...
if simulate:
    ...
    while results.is_processing():
        ...
        ax[0, 0].set_title('q1 - I')
        ax[0, 0].set_ylabel("Interaction time (ns)")
        ax[1, 0].cla()
        ax[1, 0].pcolor(amps, 4 * ts, Q1)
        ax[1, 0].set_title('q1 - Q')
        ax[1, 0].set_xlabel("FLux amplitude (V)")
        ax[1, 0].set_ylabel("Interaction time (ns)")
        ax[0, 1].cla()
        ax[0, 1].pcolor(amps, 4 * ts, I2)
        ax[0, 1].set_title('q2 - I')
        ax[1, 1].cla()
        ax[1, 1].pcolor(amps, 4 * ts, Q2)
        ax[1, 1].set_title('q2 - Q')
        ax[1, 1].set_xlabel("FLux amplitude (V)")
        plt.tight_layout()
        plt.pause(0.1)
