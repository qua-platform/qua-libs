...
if simulate:
    ...
    while results.is_processing():
        ...
        ax[0, 0].cla()
        ax[0, 0].plot(4 * idle_times, I1)
        ax[0, 0].set_title("I1")
        ax[1, 0].cla()
        ax[1, 0].plot(4 * idle_times, Q1)
        ax[1, 0].set_title("Q1")
        ax[1, 0].set_xlabel("idle_times (ns)")
        ax[0, 1].cla()
        ax[0, 1].plot(4 * idle_times, I2)
        ax[0, 1].set_title("I2")
        ax[1, 1].cla()
        ax[1, 1].plot(4 * idle_times, Q2)
        ax[1, 1].set_title("Q2")
        ax[1, 1].set_xlabel("idle_times (ns)")
        plt.tight_layout()
        plt.pause(0.1)
...
try:
    fit = Fit()
    plt.figure()
    fit.ramsey(4 * idle_times * 1e-9, I1, plot=True, verbose=True)
except (Exception, ):
    pass
