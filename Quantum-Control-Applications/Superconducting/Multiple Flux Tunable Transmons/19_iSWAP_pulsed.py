    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        with for_(*from_array(a, amps)):
            with for_(segment, 0, segment <= const_flux_len, segment + 1):
                play("x180", "q1_xy")
                align()
                wait(4)  # TODO: to guarantee that flux and drive don't overlap, can be calibrated
...
#####################################
qmm = QuantumMachinesManager(host=qop_ip, port=83)
...
if simulate:
    ...
    fig = plot_simulator_output(job, config, duration_ns=11000, plot_axes=[["q1_xy"], ["q1_z"], ["rr1"], ["rr2"]])
    fig.show()
else:
