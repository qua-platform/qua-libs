...
with program() as power_rabi:
    ...
    with stream_processing():
        for q in range(len(qubit_list)):
            I_st[q].buffer(len(amps)).average().save(f"I{q}")
            Q_st[q].buffer(len(amps)).average().save(f"Q{q}")
    ...
    for q in range(len(qubit_list)):
        ...
        while my_results.is_processing() and qubit_data[q]["iteration"] < n_avg - 1:
            ...
            if debug and not fit_data:
                ...
                if fit_data:
                    try:
                        Fit.rabi(amps * machine.get_qubit_gate(q, gate_shape).angle2volt.deg180, qubit_data[q]["I"])
                        plt.subplot(211)
                        plt.cla()
                        fit_I = Fit.rabi(
                            amps * machine.get_qubit_gate(q, gate_shape).angle2volt.deg180,
                        ...
                        pass
                    try:
                        Fit.rabi(amps * machine.get_qubit_gate(q, gate_shape).angle2volt.deg180, qubit_data[q]["Q"])
                        plt.subplot(211)
                        plt.cla()
                        fit_Q = Fit.rabi(
                            amps * machine.get_qubit_gate(q, gate_shape).angle2volt.deg180,
