...
def active_reset(machine, threshold, qubit_index, max_tries=1, Ig=None):
    """Macro for performing active reset until successful for a given number of tries.
    ...
    with while_((Ig > threshold) & (counter < max_tries)):
        ...
        play("x180", machine.qubits[qubit_index].name, condition=(Ig > threshold))
...
def single_measurement(machine, qubit_index, threshold=None, state=None, I=None, Q=None):
    """
...
def ge_averaged_measurement(machine, qubit_index, cooldown_time, n_avg):
    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        wait(cooldown_time, machine.readout_resonators[qubit_index].name, machine.qubits[qubit_index].name)
        save(I, Ig_st)
        save(Q, Qg_st)
