...
machine.qubits[0].ramsey_det = 10e6
machine.qubits[1].ramsey_det = 10e6
...
with program() as power_rabi:
    ...
    for i in range(len(qubit_list)):
        ...
        with for_(n[i], 0, n[i] < n_avg, n[i] + 1):
            with for_(*from_array(tau, taus)):
                play("x90", machine.qubits[i].name)
                wait(tau, machine.qubits[i].name)
                frame_rotation_2pi(
                    Cast.mul_fixed_by_int(machine.qubits[i].ramsey_det * 1e-9, 4 * tau), machine.qubits[i].name
    ...
    with stream_processing():
        for i in range(len(qubit_list)):
            I_st[i].buffer(len(taus)).average().save(f"I{i}")
            Q_st[i].buffer(len(taus)).average().save(f"Q{i}")
            state_st[i].boolean_to_int().buffer(len(taus)).average().save(f"state{i}")
