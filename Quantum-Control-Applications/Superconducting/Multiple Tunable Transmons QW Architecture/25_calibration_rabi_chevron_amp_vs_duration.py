...
for q in qubit_list:
    machine.get_qubit_gate(q, gate_shape).length = gate_params[q][0]
    machine.get_qubit_gate(q, gate_shape).angle2volt.deg180 = gate_params[q][1]
...
with program() as rabi:
    ...
    with stream_processing():
        for q in range(len(qubit_list)):
            I_st[q].buffer(len(lengths)).buffer(len(amplitudes)).average().save(f"I{q}")
            Q_st[q].buffer(len(lengths)).buffer(len(amplitudes)).average().save(f"Q{q}")
