...
from qm.qua import switch_, case_, declare, align, for_, default_, wait
...
class RBBaker:
    ...
    def bake(self) -> dict:
        config = copy.deepcopy(self._config)
        import pprint
        with open(f"config_2qrb.txt", 'w') as file:
            file.write("config = ")
            pprint.pprint(config, stream=file)
    ...
    def run(self, op_list_per_qe: dict, length, unsafe=False):
        from configuration_mw_fem import PI_LEN
        ...
        for qe, op_list in op_list_per_qe.items():
            ...
            with for_(cmd_i, 0, cmd_i < length, cmd_i + 1):
                with switch_(op_list[cmd_i], unsafe=unsafe):
                    ...
                    with default_():
                        wait(PI_LEN >> 2, qe) # this would raise an error
        align()
