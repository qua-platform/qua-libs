...
class qubit_frequency_tracking:
    ...
    def time_domain_ramesy_full_sweep_frame_rot(self, reps, f_ref, tau_min, tau_max, dtau, stream_name, phase_rate, correct=False):
        ...
        with for_(self.n, 0, self.n < reps, self.n + 1):
            with for_(self.tau, tau_min, self.tau < tau_max, self.tau + dtau):
                ...
                play("x90", self.qubit)
                wait(self.tau, self.qubit)
                frame_rotation_2pi(Cast.mul_fixed_by_int(self.phase_rate, 4*self.tau), self.qubit)
                play("x90", self.qubit)
    ...
    def freq_domain_ramsey_full_sweep_frame_rot(self, reps, pmin, pmax, dp, stream_name, phase_rate, oscillation_number=1, correct=False):
        self.phase_rate = phase_rate
        self.tau0 = oscillation_number * int(1 / self.phase_rate / 4e-9)
        self.delta = 1 / (self.tau0 * 4e-9) / 4  # the last 4 is for 1/4 of a cycle
        self.pvec = np.arange(pmin, pmax, dp).astype(int).tolist()

        with for_(self.m, 0, self.m < reps, self.m + 1):
            with for_(self.p, pmin, self.p < pmax, self.p + dp):
                ...
                play("x90", self.qubit)
                wait(self.tau0, self.qubit)
                frame_rotation_2pi(Cast.mul_fixed_by_int(self.p, 4*self.tau0), self.qubit)
                play("x90", self.qubit)
