...
delta_cliffords = np.round(np.logspace(np.log10(1), np.log10(max_circuit_depth), num_points)).astype(int).tolist()
...
with program() as rb:
    ...
    d_cliffords = declare(int, value=delta_cliffords)
    r = declare(int)
    ...
    with stream_processing():
        ...
        state_st.boolean_to_int().buffer(n_avg).map(FUNCTIONS.average()).buffer(num_points).buffer(
            num_of_sequences
        ).save("res")
        # returns a 1D array of averaged random pulse sequences vs depth of circuit
        state_st.boolean_to_int().buffer(n_avg).map(FUNCTIONS.average()).buffer(num_points).average().save("res2")
...
x = np.round(np.logspace(np.log10(1), np.log10(max_circuit_depth), num_points))
