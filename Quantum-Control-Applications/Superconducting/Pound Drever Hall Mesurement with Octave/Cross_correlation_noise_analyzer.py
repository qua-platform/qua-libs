...
qm.octave.set_downconversion(
    element, lo_source=RFInputLOSource.Internal, if_mode_i=IFMode.envelope, if_mode_q=IFMode.envelope
)  # Dmd2LO
...
pound_samples_per_chunk = 1 * int(
    2 * 250e6 / pound_modulation_freq
)  # in clocks = 100ns to catch 2 periods of the PDH signal
...
with program() as noise_analyzer:
    ...
    align()
    with for_(
        pound_index, 0, pound_index < 5 * averaging_length * correlation_length * fft_averaging_length, pound_index + 1
    ):
        play("pound_pulse", "RR")
...
if simulate:
    ...
    plt.figure(figsize=(16, 12))
    plt.plot(
        [(x + 1) * pound_samples_per_chunk * 4 for x in range(corr_I.__len__())], corr_I, linewidth=5, markersize=10
    )
    plt.plot(
        [(x + 1) * pound_samples_per_chunk * 4 for x in range(corr_Q.__len__())], corr_Q, linewidth=5, markersize=10
    )
