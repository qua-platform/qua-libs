...
with program() as PDH_PID:
    ...
    with for_(n, 0, n < N_averaging, n + 1):
        with for_(ind, 0, ind < PID_length, ind + 1):
            ...
            assign(
                integrator_error,
                Cast.mul_int_by_fixed(integrator_error, 1 - alpha) + Cast.mul_int_by_fixed(error, alpha),
            )
            assign(derivative_error, old_error - error)
            assign(
                f,
                f
                + Cast.mul_int_by_fixed(error, gain_P)
    ...
    with stream_processing():
        ...
        pound_signal_st.buffer(averaging_length_for_plotting).average().save(
            "pound_signal"
        )  # buffer it into vectors the length of the frequency scan and average over the averaging loop
