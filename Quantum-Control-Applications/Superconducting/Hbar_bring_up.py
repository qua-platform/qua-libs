...
config = {
    ...
    "elements": {
        ...
        "qubit2": {
            ...
            "intermediate_frequency": qubit_if+10e6,
            "operations": {
                "gaussian": "gaussian_pulse",
            },
            "thread": "b",
    ...
    "pulses": {
        "gaussian_pulse": {
            "operation": "control",
            "length": g_len,
            #"length": int(g_len*sampling_rate/1e9),
            "waveforms": {
                "single": "gaussian_wf",
            },
        },
...
with program() as demo:
    with infinite_loop_():
        play("gaussian", "qubit", duration=g_len * u.ns)
        play("gaussian", "qubit2", duration=g_len * u.ns)
        #update_frequency("qubit", 30e6)
        #play("gaussian", "qubit")
        #update_frequency("qubit", qubit_if)
        #play("gaussian" * amp(update_amp), "qubit", duration = update_duration, truncate = cut_pulse)#phase/frame rotation
        #play(ramp(0.0001), "qubit", duration = 2 * u.us) #ramp rate in V/ns
...
if simulate:
    ...
    simulation_config = SimulationConfig(duration=1_500)  # In clock cycles = 4ns
