...
resonator.readout_params.readout_length = 50e-06
resonator.readout_params.readout_amplitude = 0.1
...
cooldown_time = 10 * u.us // 4  # Resonator cooldown time in clock cycles (4ns)

f_min = resonator.frequency - resonator.lo - 5e6
f_max = resonator.frequency - resonator.lo + 5e6
df = 0.5e6
...
with program() as resonator_spec:
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(f, f_min, f <= f_max, f + df):  # Notice it's <= to include f_max (This is only for integers!)
            update_frequency(resonator.name, f)
            measure(
                "readout",
                resonator.name,
                None,
...
if simulate:
    ...
    job = qmm.simulate(build_config(machine), resonator_spec, simulation_config)
    ...
    I = u.demod2volts(I, resonator.readout_params.readout_length)
    Q = u.demod2volts(Q, resonator.readout_params.readout_length)
...
# update parameters
ready = False
resonator.frequency = resonator.lo + 100e6
