    ...
    with for_(n, 0, n < n_avg, n + 1):
        ...
        with for_(*from_array(df, dfs)):
            ...
            with for_(*from_array(dc, dcs)):
                # Flux sweeping
                set_dc_offset(qb1.qubit_name + "_z", "single", dc)
                set_dc_offset(qb2.qubit_name + "_z", "single", dc)
                wait(100)  # Wait for the flux to settle

                # Saturate qubit
                # play("cw" * amp(1), qb1.qubit_name + "_xy", duration=t * u.ns)
                # play("cw" * amp(1), qb2.qubit_name + "_xy", duration=t * u.ns)
...
if simulate:
    ...
    with qm_session(qmm, config, timeout=100) as qm:
        ...
        while results.is_processing():
            ...
            plt.xlabel("Flux [V]")
            plt.ylabel(f"{qb1.qubit_name} IF [MHz]")
            plt.title(f"{qb1.qubit_name} (f_01: {int(qb1.xy.f_01 / u.MHz)} MHz)")
            plt.subplot(223)
            plt.cla()
            ...
            plt.title(f"{qb2.qubit_name} (f_01: {int(qb2.xy.f_01 / u.MHz)} MHz)")
            plt.ylabel(f"{qb2.qubit_name} IF [MHz]")
            plt.xlabel("flux [V]")
            plt.subplot(224)
            plt.cla()
