...
with program() as red_sideband:
    ...
    a = declare(fixed, value=rs_a)  # Update the amplitude of the flux pulse directly in QUA
    ...
    with for_(n, 0, n < n_avg, n + 1):
        with for_(*from_array(tau, times)):
            with for_(*from_array(f, freqs)):
                ...
                update_frequency("flux_line_rs", f)
                # Wait for the qubit to decay
                wait(cooldown_time, "qubit1", "flux_line_rs")
                # drive the qubit to the |e> state
                play("pi" * amp(q1_ge_amp), "qubit1")
                # Play the red sideband after the qubit pulse with varying duration
                align("qubit1", "flux_line_rs")
                play("offset" * amp(a), "flux_line_rs", duration=tau)
                # Measure the state of the resonator after the flux pulse
                align("resonator1", "flux_line_rs")
                measure(
                    "readout",
                    "resonator1",
                    None,
...
if simulate:
    ...
    job = qmm.simulate(config, red_sideband, simulation_config)  # do simulation with qmm
    ...
    while results.is_processing():
        ...
        # Plot results
        plt.cla()
        plt.title("Red sideband chevron pattern")
