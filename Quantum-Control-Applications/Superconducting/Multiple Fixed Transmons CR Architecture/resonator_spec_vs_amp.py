"""
resonator_spec_vs_amp.py:performs 2D scans to all qubits of resonator spectroscopy vs the readout amplitude.
"""
...
with program() as resonator_spec_vs_amp:
    n = declare(int)
    f = declare(int)
    a = declare(fixed)
    I = declare(fixed)
...
if simulate:
    ...
    job = qmm.simulate(config, resonator_spec_vs_amp, simulation_config)
    job.get_simulated_samples().con1.plot()

else:
    qm = qmm.open_qm(config)
    job = qm.execute(resonator_spec_vs_amp)
...
for q in qubits:
    ...
    plt.subplot(x)
    plt.cla()
    plt.title(f"rr{q} spectroscopy vs amplitude - amplitude")
    ...
    plt.title(f"rr{q} spectroscopy vs amplitude - phase")
    plt.pcolor(phase)
    # plt.xlabel("frequency [MHz]")
