...
nodeio.context(name="AmpRabiErrAmplification", description="performs the 1D amp rabi for multiple qubits")

inputs = nodeio.Inputs()
inputs.stream("state", description="system state", units="JSON")
inputs.stream(
    "progressive_calibration",
    description="strategy for progressive calibration rounds",
    units="JSON",
)
...
outputs.define(
    "state",
    description="system state after this calibration",
    units="JSON",
    retention=2,
)
...
# ==================== DRY RUN DATA ====================
from state_and_config import build_config
from state_and_config import state as bootstrap_state

# set inputs data for dry-run of the node
inputs.set(state=bootstrap_state)
# operations: 1 = x180
# operations: 2 = two times y90
inputs.set(
    progressive_calibration=[
        {"repeated_pulses": 3, "operations": 1},
        {"repeated_pulses": 5, "operations": 2},
    ]
)
...
while nodeio.status.active:
    ...
    progressive_calibration = inputs.get("progressive_calibration")

    # === progressive calibration while loop START
    for cal in progressive_calibration:
            ...
            for q in qubits:
                ...
                with for_(n, 0, n < n_avg, n + 1):
                    with for_(
                        ...
                        with for_(m, 0, m < cal["repeated_pulses"], m + 1):
                            option = declare(int, value=cal["operation"])
                            with switch_(option):
                                with case_(1):
                                    play("x180" * amp(a), f"q{q}")
                                with case_(2):
                                    play("y90" * amp(a), f"q{q}")
                                    play("y90" * amp(a), f"q{q}")
                        align()
        ...
        for q in qubits:
            ...
            plt.subplot(x)
            plt.cla()
            plt.title(f"q{q} amp rabi, X{repeated_pulses} ERR amplification - I")
            plt.plot(amps, I, ".")
    ...
    # === progressive calibration for loop END
