...
with program() as PROGRAM:
    ...
    with stream_processing():
        ...
        state_st[0].boolean_to_int().buffer(2).buffer(3).buffer(len(ts_cycles)).buffer(len(amp_scalings)).average().save("state1")
        # target qubit
        ...
        state_st[1].boolean_to_int().buffer(2).buffer(3).buffer(len(ts_cycles)).buffer(len(amp_scalings)).average().save("state2")
