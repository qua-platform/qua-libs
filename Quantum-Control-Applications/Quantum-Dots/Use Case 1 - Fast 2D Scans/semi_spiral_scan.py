...
n_points_slow = 10
n_points_fast = 101
voltage_slow_min = -0.3
voltage_slow_max = 0.3
voltage_fast_min = -0.2
voltage_fast_max = 0.2

def bias_t_comp(start, end, n):
    ...
    half = (n + 1) // 2
    result[0::2] = arr[:half]
    result[1::2] = arr[:half-1:-1]
    
    return result

# Voltages in Volt
voltage_values_slow = bias_t_comp(voltage_slow_min, voltage_slow_max, n_points_slow)
...
voltage_values_fast = np.linspace(voltage_fast_min, voltage_fast_max, n_points_fast)
...
#, _ = get_filtered_voltage(voltage_values_fast, step_duration=1e-6, bias_tee_cut_off_frequency=1e3, plot=True)
...
with program() as charge_stability_prog:
    ...
    with for_(n, 0, n < n_avg, n + 1):  # The averaging loop
        with for_(*from_array(Vslow, voltage_values_slow)):
