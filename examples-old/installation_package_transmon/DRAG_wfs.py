...
def cos_detuned(amplitude, delf, length, alph, delt):
    ...
    # Detuning correction Eqn. (4) in Chen et al. PRL, 116, 020501 (2016)
    cos_wave = cos_wave * np.cos(2 * np.pi * delf * t * 1e-9) - (
        alph / delt
    ) * sin_wave * np.sin(2 * np.pi * delf * t * 1e-9)
...
def cos_derivative_detuned(amplitude, delf, length, alph, delt):
    ...
    # Detuning correction Eqn. (4) in Chen et al. PRL, 116, 020501 (2016)
    cos_derivative_detuned_wave = (alph / delt) * sin_wave * np.cos(
        2 * np.pi * delf * t * 1e-9
    ) + cos_wave * np.sin(2 * np.pi * delf * t * 1e-9)
...
drag_gauss_wf = gaussian_detuned(
    drag_amp, drag_len / 5, del_f, drag_len, alpha, delta
)  # pi pulse
drag_half_gauss_wf = gaussian_detuned(
    drag_amp * 0.5, drag_len / 5, del_f, drag_len, alpha, delta
)  # pi_half pulse
minus_drag_half_gauss_wf = gaussian_detuned(
    drag_amp * (-0.5), drag_len / 5, del_f, drag_len, alpha, delta
)  # -pi_half pulse
minus_drag_gauss_wf = gaussian_detuned(
...
drag_gauss_der_wf = gaussian_derivative_detuned(
    drag_amp, drag_len / 5, del_f, drag_len, alpha, delta
)  # pi pulse
...
drag_half_cos_wf = cos_detuned(
    drag_amp * 0.5, del_f, drag_len, alpha, delta
)  # pi_half pulse
minus_drag_half_cos_wf = cos_detuned(
    drag_amp * (-0.5), del_f, drag_len, alpha, delta
)  # -pi_half pulse
minus_drag_cos_wf = cos_detuned(
    drag_amp * (-1), del_f, drag_len, alpha, delta
)  # -pi pulse

# Definition of Q-quadrature DRAG waveforms for pi and pi_half pulses with a cosine envelope
drag_sin_wf = cos_derivative_detuned(
    drag_amp, del_f, drag_len, alpha, delta
)  # pi pulse
drag_half_sin_wf = cos_derivative_detuned(
    drag_amp * 0.5, del_f, drag_len, alpha, delta
)  # pi_half pulse
minus_drag_half_sin_wf = cos_derivative_detuned(
    drag_amp * (-0.5), del_f, drag_len, alpha, delta
)  # -pi_half pulse
minus_drag_sin_wf = cos_derivative_detuned(
    drag_amp * (-1), del_f, drag_len, alpha, delta
)  # -pi pulse
