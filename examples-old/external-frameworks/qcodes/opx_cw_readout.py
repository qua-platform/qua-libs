...
class OPXCWReadout(OPX):
    ...
    def get_prog(self):
        ...
        with program() as prog:
            ...
            with stream_processing():
                I_st.buffer(n_avg).map(FUNCTIONS.average()).save("I")
                Q_st.buffer(n_avg).map(FUNCTIONS.average()).save("Q")
    ...
    def get_res(self):
        if self.result_handles is None:
            ...
            time.sleep(self.t_meas() / 10)  # To verify that everything settled (Needed?)
