...
N = 1000
M = 20  # size of CPMG block (number of pi pulses)
...
# Stores the baking objects which contains the CPMG blocks of size M
cpmg_block_list = []
for i in range(t_len):  # Create the different baked sequences
    ...
    with baking(config, sampling_rate=sample_rate, padding_method="none") as b:
        b.frame_rotation_2pi(0.25, "qubit")
...
with program() as CPMG:
    ...
    play("init", "laser_AOM")
    align()
    with for_(j, 0, j < len(cpmg_block_list), j + 1):
        with switch_(j, unsafe=True):
            for i in range(len(cpmg_block_list)):
                with case_(i):
                    play("pi_half", "qubit")
                    with for_(k, 0, k < N / M - 1, k + 1):
                        cpmg_block_list[i].run()
    ...
    with stream_processing():
        counts_st.buffer(len(cpmg_block_list)).save("counts")

qmm = QuantumMachinesManager()
job = qmm.simulate(
    config,
    CPMG,
    SimulationConfig(int((9 * t_max * N) / 4)),
)
plt.figure()
...
peaks = find_peaks(channel_1, plateau_size=2)[0]
for i in range(4):
    first_peak = peaks[2 * i]
    t = t_vec[i]
    # Asserts that the sequence has the correct duration
    assert first_peak + (2 * N) * t == peaks[2 * i + 1]
    ...
    plt.vlines(first_peak + (2 * N) * t, 0, 0.45)
